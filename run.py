...
class CLIHandler:
    def __init__(self):
        self.parser = argparse.ArgumentParser(
            description="Run integration tests with or without fast token delivery."
        )
        self.add_arguments()
    ...
    def add_arguments(self):
        self.parser.add_argument(
            "--ftd",
            action="store_true",
            help="Run tests with fast token delivery (default: run without FTD).",
        )
        self.parser.add_argument(
            "--sq",
            action="store_true",
            help="Use straight-to-queue with FTD (implies --ftd).",
        )
        self.parser.add_argument(
            "--no-ftd",
            action="store_true",
            help="Explicitly run tests without fast token delivery (the default).",
        )

    def parse_args(self, args=None):
        parsed_args = self.parser.parse_args(args)
        if parsed_args.no_ftd and (parsed_args.ftd or parsed_args.sq):
            self.parser.error("--no-ftd cannot be combined with --ftd or --sq")
        return parsed_args
...
def main():
    ...
    use_ftd = args.ftd or args.sq
    dyn_file = FTD_DYN_FILE if use_ftd else DYN_FILE
    write_string_to_file(SCRIPT_CONTENT, dyn_file)

    if args.sq:
        mode = "WITH FAST TOKEN DELIVERY AND STRAIGHT TO QUEUE"
        modify_line(dyn_file, FTD_COMPILE_COMMAND_SQ, 2)
    elif use_ftd:
        mode = "WITH FAST TOKEN DELIVERY"
        modify_line(dyn_file, FTD_COMPILE_COMMAND, 2)
    else:
        mode = "WITHOUT FAST TOKEN DELIVERY"
