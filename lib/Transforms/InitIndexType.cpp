    ...
    if (isa<mlir::arith::IndexCastOp>(op)){
      ...
      if (!isResIndType){
        if (indexCastOp.getResult().getType().getIntOrFloatBitWidth() == 
          ...
          auto newOp = insertWidthMatchOp(&op, 0, op.getResult(0).getType(), ctx);
          if (newOp.has_value())
      ...
      if (isResIndType && !isOpIndType){
        if (indexCastOp.getOperand().getType().getIntOrFloatBitWidth() == indexWidth)
          indexCastOp.getResult().replaceAllUsesWith(indexCastOp.getOperand());
        else {
          builder.setInsertionPoint(&op);
          auto extOp = builder.create<mlir::arith::ExtSIOp>(op.getLoc(), 
