...
// Helper functions
  ...
  // If the condition value is driven by a NOT, return the original condition feeding the NOT
  // Otherwise, return the passed condition as is, and set the flag parameter to false
  Value returnNonNegatedCondition(Value condition, bool& isNegated) {
    if (!isa_and_nonnull<handshake::NotOp>(condition.getDefiningOp())) {
      isNegated = false;
      return condition;
    }
    isNegated = true;
    return condition.getDefiningOp()->getOperand(0);
  }
  ...
  // Rules A Removes Conditional Branch and muxOrMerge
  // operation pairs if both the inputs of the muxOrMerge are outputs of the
  // Conditional Branch. The results of the muxOrMerge are replaced with the data
  // operand of the Conditional Branch.
  template <typename MuxOrMergeOp>
  struct RemoveRedunIfThenElse
    : public OpRewritePattern <MuxOrMergeOp> {
    using OpRewritePattern<MuxOrMergeOp>::OpRewritePattern;
    ...
    LogicalResult matchAndRewrite(MuxOrMergeOp muxOrMergeOp,
      ...
      // The two operands should be conditional Branches; otherwise,
      // the pattern match fails
      Operation* firstOperand = muxOrMergeOperands[0].getDefiningOp();
      Operation* secondOperand = muxOrMergeOperands[1].getDefiningOp();
      ...
      bool isNegatedfirstBranchCondition;
      Value firstBranchOriginalConditon = returnNonNegatedCondition(firstBranchOperand.getConditionOperand(), isNegatedfirstBranchCondition);
      bool isNegatedsecondBranchCondition;
      Value secondBranchOriginalConditon = returnNonNegatedCondition(secondBranchOperand.getConditionOperand(), isNegatedsecondBranchCondition);

      // If the two original conditions are not equivalent or not 1 of them is negated, the pattern match
      // fails
      if (firstBranchOriginalConditon != secondBranchOriginalConditon ||
        isNegatedfirstBranchCondition == isNegatedsecondBranchCondition)
        return failure();
      ...
      Value muxOrMergeOutput = muxOrMergeOp.getResult();

      // Replace all uses of the merge output with the input of the Branches
      rewriter.replaceAllUsesWith(muxOrMergeOutput, firstBranchData);
      rewriter.eraseOp(muxOrMergeOp);
  ...
  template <typename MuxOrMergeOp>
  struct RemoveRedunLoop : public OpRewritePattern<MuxOrMergeOp> {
    using OpRewritePattern<MuxOrMergeOp>::OpRewritePattern;
    ...
    LogicalResult matchAndRewrite(MuxOrMergeOp muxOrMergeOp,
      ...
      assert(isa_and_nonnull<handshake::ConditionalBranchOp>(potentialBranchOp));
      handshake::ConditionalBranchOp loopBranchOp =
        cast<handshake::ConditionalBranchOp>(potentialBranchOp);

      int outsideInputIdx = 1 - cycleInputIdx;

      // Another user of the muxOrMerge should be another Branch that has the same condition as that of loopBranch
      bool foundExitBranch = false;
      Operation* potentialLoopExitBranch = nullptr;
      for (auto user : users) {
        if (isa_and_nonnull<handshake::ConditionalBranchOp>(user) && user != potentialBranchOp) {
          foundExitBranch = true;
          potentialLoopExitBranch = user;
          break;
        }
      }

      if (potentialLoopExitBranch == nullptr)
        return failure();

      assert(isa_and_nonnull<handshake::ConditionalBranchOp>(potentialLoopExitBranch));
      handshake::ConditionalBranchOp loopExitBranchOp =
        cast<handshake::ConditionalBranchOp>(potentialLoopExitBranch);

      if (!isSuppress(loopBranchOp) || !isSuppress(loopExitBranchOp))
        return failure();

      bool isNegatedLoopBranchCondition;
      Value loopBranchOriginalConditon = returnNonNegatedCondition(loopBranchOp.getConditionOperand(), isNegatedLoopBranchCondition);
      bool isNegatedLoopExitBranchCondition;
      Value loopExitBranchOriginalConditon = returnNonNegatedCondition(loopExitBranchOp.getConditionOperand(), isNegatedLoopExitBranchCondition);

      // If the two original conditions are not equivalent or not 1 of them is negated, the pattern match
      // fails
      if (loopBranchOriginalConditon != loopExitBranchOriginalConditon ||
        isNegatedLoopBranchCondition == isNegatedLoopExitBranchCondition)
        return failure();
      ...
      rewriter.replaceAllUsesWith(
        loopExitBranchOp.getFalseResult(), muxOrMergeOperands[outsideInputIdx]);
  ...
  struct ExtractLoopCondition
    ...
    LogicalResult matchAndRewrite(handshake::ControlMergeOp cmergeOp,
      ...
      // Another user of the cmerge should be another Branch that has the same condition as that of loopBranch
      bool foundExitBranch = false;
      Operation* potentialLoopExitBranch = nullptr;
      for (auto user : cmergeUsers) {
        if (isa_and_nonnull<handshake::ConditionalBranchOp>(user) && user != potentialBranchOp) {
          foundExitBranch = true;
          potentialLoopExitBranch = user;
          break;
      ...
      Value condition = loopBranchOp.getConditionOperand();
      ...
      if (needNot) {
        if (foundNot)
          ...
          rewriter.setInsertionPoint(loopBranchOp);
          handshake::NotOp notOp = rewriter.create<handshake::NotOp>(
            loopBranchOp->getLoc(), condition);
          inheritBB(notOp, loopBranchOp);
      ...
      if (foundInit) {
        ...
        Value valueOfConstant = rewriter.create<handshake::ConstantOp>(
          loopBranchOp->getLoc(), constantType,
          rewriter.getIntegerAttr(constantType, constVal), start);

        // Add a new Merge operation to serve as the INIT
  ...
  struct ExtractIfThenElseCondition
    ...
    LogicalResult matchAndRewrite(handshake::ControlMergeOp cmergeOp,
      ...
      if (!isSuppress(firstBranchOperand) ||
        !isSuppress(secondBranchOperand))
        return failure();
      ...
      // the index should be driven by the branch condition or its negation
      // if the suppress feeding in0 of the cmerge is negated and that feeding in1 is not negated,
      // the index output will be the negation of the branch condition
      bool needNot = (isNegatedfirstBranchCondition && !isNegatedsecondBranchCondition);

      Value condToReplaceIndex;
      if (needNot) {
        Operation* potentialNotOp = isConditionInverted(firstBranchOriginalConditon);
        assert(potentialNotOp != nullptr && isa_and_nonnull<handshake::NotOp>(potentialNotOp));
        handshake::NotOp existingNotOp = cast<handshake::NotOp>(potentialNotOp);
        condToReplaceIndex = existingNotOp.getResult();
      }
      else
        condToReplaceIndex = firstBranchOriginalConditon;

      rewriter.replaceAllUsesWith(index, condToReplaceIndex);
  ...
  struct HandshakeRewriteTermsPass
    ...
    void runDynamaticPass() override {
      ...
      patterns.add<
        ...
        ConstructSuppresses, FixBranchesToSuppresses,
        DistributeSuppresses,
        DistributeRepeats<handshake::MuxOp>,
        DistributeRepeats<handshake::MergeOp>,
        ExtractIfThenElseCondition, ExtractLoopCondition,
        RemoveRedunIfThenElse<handshake::MuxOp>, RemoveRedunIfThenElse<handshake::MergeOp>,
        RemoveRedunLoop<handshake::MergeOp>, RemoveRedunLoop<handshake::MuxOp> /*, ShortenSuppressPairs,*/
