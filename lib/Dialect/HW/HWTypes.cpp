...
using namespace dynamatic::hw::detail;

#define GET_TYPEDEF_CLASSES
#include "dynamatic/Dialect/HW/HWTypes.cpp.inc"

//===----------------------------------------------------------------------===//
// Type Helpers
//===----------------------------------------------------------------------===/

mlir::Type dynamatic::hw::getCanonicalType(mlir::Type type) {
  Type canonicalType;
...
bool dynamatic::hw::isHWIntegerType(mlir::Type type) {
...
bool dynamatic::hw::isHWEnumType(mlir::Type type) {
...
bool dynamatic::hw::isHWValueType(Type type) {
...
/// statically computed.
int64_t dynamatic::hw::getBitWidth(mlir::Type type) {
...
bool dynamatic::hw::hasHWInOutType(Type type) {
  if (auto array = dyn_cast<ArrayType>(type))
...
Type IntType::get(mlir::TypedAttr width) {
  ...
  auto widthWidth = dyn_cast<IntegerType>(width.getType());
  assert(widthWidth && widthWidth.getWidth() == 32 &&
         "!hw.int width must be 32-bits");
  (void)widthWidth;

  if (auto cstWidth = dyn_cast<IntegerAttr>(width))
...
void EnumType::print(AsmPrinter &p) const {
  ...
  llvm::interleaveComma(getFields(), p, [&](Attribute enumerator) {
    p << cast<StringAttr>(enumerator).getValue();
  });
  p << ">";
}
...
std::optional<size_t> EnumType::indexOf(mlir::StringRef field) {
  for (auto it : llvm::enumerate(getFields()))
    if (cast<StringAttr>(it.value()).getValue() == field)
      return it.index();
  return {};
}
...
size_t ArrayType::getNumElements() const {
  if (auto intAttr = dyn_cast<IntegerAttr>(getSizeAttr()))
...
ModuleType dynamatic::hw::detail::fnToMod(Operation *op,
...
ModuleType dynamatic::hw::detail::fnToMod(FunctionType fnty,
                                          ArrayRef<Attribute> inputNames,
                                          ArrayRef<Attribute> outputNames) {
