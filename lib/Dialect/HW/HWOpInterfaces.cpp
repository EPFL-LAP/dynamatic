...
hw::InnerSymAttr hw::PortInfo::getSym() const {
  if (attrs)
    return attrs.getAs<::dynamatic::hw::InnerSymAttr>(
...
raw_ostream &dynamatic::hw::operator<<(raw_ostream &printer, PortInfo port) {
  StringRef dirstr;
  switch (port.dir) {
...
#include "dynamatic/Dialect/HW/HWOpInterfaces.cpp.inc"
