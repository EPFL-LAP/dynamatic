...
void BLIFGenerator::copyAndRenameBlif(
    const std::string &srcDir, const std::string &srcComp,
    const std::string &dstComp,
    const std::vector<std::string> &paramValues) const {
  namespace fs = std::filesystem;

  std::string srcFile = srcDir + "/" + srcComp + ".blif";
  if (!fs::exists(srcFile)) {
    llvm::errs() << "BLIFGenerator source BLIF not found: " << srcFile << "\n";
    return;
  }

  // Target: blifDirPath/<dstComp>/<p0>/<p1>/.../
  fs::path dstDir = blifDirPath;
  dstDir /= dstComp;
  for (const auto &p : paramValues)
    dstDir /= p;
  fs::create_directories(dstDir);

  std::string dstFile = (dstDir / (dstComp + ".blif")).string();
  ...
  // Replace every occurrence of srcComp with dstComp.
  std::string result;
  size_t pos = 0;
  while (true) {
    size_t found = content.find(srcComp, pos);
    if (found == std::string::npos) {
      result += content.substr(pos);
      break;
    }
    result += content.substr(pos, found - pos) + dstComp;
    pos = found + srcComp.size();
  }
