...
BackendGenerator::BackendGenerator(Backend backend, BackendParams params)
...
bool BackendGenerator::generateBLIF(mlir::Operation *op) {
  ...
  const std::string &rtlConfigPath = params.rtlConfigPath;
  const std::string &dynamaticRoot = params.dynamaticRoot;
  const std::string &outputBaseDir = params.outputBaseDir;
  ...
  BackendGenerator verilogGen(Backend::Verilog, params);
  ...
  std::string outDirPath = getOutputDirForOp(op, outputBaseDir);
  ...
  if (!runYosys(moduleName, outDirPath, verilogFiles, yosysOutName)) {
  ...
  if (!runAbc(yosysOutPath, outDirPath, abcOutName)) {
...
bool BackendGenerator::generateVerilog(mlir::Operation *op) {
  ...
  std::map<std::string, std::string> rtlParamMap;
  getParamMap(op, rtlParamMap);
  ...
  rtlParamMap["MODULE_NAME"] = moduleName;
  rtlParamMap["OUTPUT_DIR"] = outDirPath;
  rtlParamMap["DYNAMATIC"] = dynamaticRoot;
  rtlParamMap["EXTRA_SIGNALS"] = "0";
  if (rtlParamMap.find("BITWIDTH") == rtlParamMap.end())
    rtlParamMap["BITWIDTH"] = "0";
  ...
  for (const auto &entryVal : *allEntries) {
    ...
    if (!nameOpt || *nameOpt != opName)
      continue;
    if (entryMatchesParams(*obj, rtlParamMap)) {
      matchedEntry = obj;
      break;
    }
  }
  ...
  std::vector<std::pair<std::string, std::string>> sortedParams(
      rtlParamMap.begin(), rtlParamMap.end());
...
bool BackendGenerator::runYosys(const std::string &topModule,
...
bool BackendGenerator::runAbc(const std::string &inputFile,
                              const std::string &outputDir,
                              const std::string &outputName) const {
