    ...
    for unit_info in dataflow_units:
        if not "name" in unit_info:
            ...
            if "dependencies" in unit_info:
                ...
                dependencies = []
            dependency_dict[unit_name] = {
                "RTL": rtl_file, "dependencies": dependencies}
...
def run_characterization(json_input, json_output, dynamatic_dir, synth_tool, clock_period, clean=False):
    ...
    for unit_info in dataflow_units:
        ...
        unit_name, list_params, generic, generator, dependencies = extract_rtl_info(
            unit_info)
        if unit_name == None:
        ...
        top_def_file = get_hdl_files(
            unit_name, generic, generator, hdl_dir, dynamatic_dir, all_dependencies_dict)
        # After generating the HDL files, we can proceed with characterization
        list_unit_chars = run_unit_characterization(unit_name, list_params, hdl_dir, synth_tool, top_def_file,
                                                    tcl_dir, rpt_dir, log_dir, clock_period, generator=generator, dynamatic_dir=dynamatic_dir)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run characterization of dataflow units")
    parser.add_argument(
        "--json-input",
        type=str,
        default=None,
