...
def WrapAdd(out, in_a, in_b, max: int) -> str:
    ...
    str_ret += '\t'*tabLevel + \
        f'-- WrapAdd({out.name}, {in_a.name}, {in_b.name}, {max})\n'
    if (isPow2(max)):
        str_ret += '\t'*tabLevel + f'{out.getNameWrite()} <= ' + \
...
def WrapAddConst(out, in_a, const: int, max: int) -> str:
    ...
    str_ret = '\t'*tabLevel + '-- WrapAdd Begin\n'
    str_ret += '\t'*tabLevel + \
        f'-- WrapAdd({out.name}, {in_a.name}, {const}, {max})\n'
    if (isPow2(max)):
        str_ret += '\t'*tabLevel + f'{out.getNameWrite()} <= ' + \
...
def WrapSub(out, in_a, in_b, max: int) -> str:
    ...
    str_ret += '\t'*tabLevel + \
        f'-- WrapSub({out.name}, {in_a.name}, {in_b.name}, {max})\n'
    if (isPow2(max)):
        str_ret += '\t'*tabLevel + f'{out.getNameWrite()} <= ' + \
            f'std_logic_vector(unsigned({in_a.getNameRead()}) - unsigned({in_b.getNameRead()}));\n'
    else:
...
def CyclicLeftShift(dout, din, distance) -> str:
    ...
    str_ret += '\t'*tabLevel + \
        f'-- CyclicLeftShift({dout.name}, {din.name}, {distance.name})\n'
...
def Reduce(dout, din, operator, comment: bool = True) -> str:
    ...
    if (comment):
        ...
        str_ret += '\t'*tabLevel + \
            f'-- Reduce({dout.name}, {din.name}, {operator})\n'
...
def Mux1H(dout, din, sel, j=None) -> str:
    ...
    str_ret += '\t'*tabLevel + \
        f'-- Mux1H({dout.name}, {din.name}, {sel.name})\n'
    useTemp()
...
def MuxLookUp(dout, din, sel) -> str:
    ...
    str_ret += '\t'*tabLevel + \
        f'-- MuxLookUp({dout.name}, {din.name}, {sel.name})\n'
...
def CyclicPriorityMasking(dout, din, base, reverse=False) -> str:
    ...
    str_ret += '\t'*tabLevel + \
        f'-- CyclicPriorityMask({dout.name}, {din.name}, {base.name})\n'
    useTemp()
    if (type(din) == LogicVecArray):
        ...
        for i in range(0, din.size):
            ...
            str_ret += Op(double_out, double_in, 'and', 'not',
                          'std_logic_vector(', 'unsigned(', double_in, ')', '-', 'unsigned(', (0,
                                                                                               size), '&', base, ')', ')'
                          )
            for j in range(0, size):
...
def PortToQueueDispatcher(
    ...
    if (numPorts != 1):
        entry_port_idx_i = LogicVecArray(
            'entry_port_idx', 'i', numEntries, portAddrW)
    ...
    entry_port_valid = LogicVecArray(
        'entry_port_valid', 'w', numEntries, numPorts)
    ...
    for i in range(0, numEntries):
        arch += Op(entry_request_valid[i], entry_valid_i[i],
    ...
    entry_port_request = LogicVecArray(
        'entry_port_request', 'w', numEntries, numPorts)
    for i in range(0, numEntries):
        arch += Op(entry_port_request[i], entry_port_valid[i],
    ...
    arch += CyclicPriorityMasking(entry_port_hs,
...
def QueueToPortDispatcher(
    ...
    entry_port_request_prio = LogicVecArray(
        'entry_port_request_prio', 'w', numEntries, numPorts)
    arch += CyclicPriorityMasking(entry_port_request_prio,
                                  entry_port_request, queue_head_oh_i)

    if (bitsW != 0):
        for j in range(0, numPorts):
            arch += Mux1H(port_bits_o[j], entry_bits_i,
                          entry_port_request_prio, j)

    # Mask the matrix with dataValid
    entry_port_request_valid = LogicVecArray(
        'entry_port_request_valid', 'w', numEntries, numPorts)
    for i in range(0, numEntries):
...
def GroupAllocator(path_rtl: str, name: str, suffix: str, configs: Configs) -> str:
    ...
    if (configs.ldpAddrW > 0):
        ldq_port_idx_o = LogicVecArray(
            'ldq_port_idx', 'o', configs.numLdqEntries, configs.ldpAddrW)
    ...
    if (configs.stpAddrW > 0):
        stq_port_idx_o = LogicVecArray(
            'stq_port_idx', 'o', configs.numStqEntries, configs.stpAddrW)

    ga_ls_order_o = LogicVecArray(
        'ga_ls_order', 'o', configs.numLdqEntries, configs.numStqEntries)
    ...
    if (configs.gaMulti):
        ...
        ga_rr_mask.regInit()
        for i in range(0, configs.numGroups):
            arch += Op(group_init_and[i], group_init_ready[i],
                       'and', group_init_valid_i[i])
            arch += Op(group_init_ready_o[i], group_init_hs[i])
        arch += CyclicPriorityMasking(group_init_hs,
                                      group_init_and, ga_rr_mask)
        for i in range(0, configs.numGroups):
            arch += Op((ga_rr_mask, (i+1) %
                       configs.numGroups), (group_init_hs, i))
    else:
        for i in range(0, configs.numGroups):
            arch += Op(group_init_ready_o[i], group_init_ready[i])
            arch += Op(group_init_hs[i], group_init_ready[i],
                       'and', group_init_valid_i[i])

    # ROM value
    if (configs.ldpAddrW > 0):
        ldq_port_idx_rom = LogicVecArray(
            'ldq_port_idx_rom', 'w', configs.numLdqEntries, configs.ldpAddrW)
    if (configs.stpAddrW > 0):
        stq_port_idx_rom = LogicVecArray(
            'stq_port_idx_rom', 'w', configs.numStqEntries, configs.stpAddrW)
    ga_ls_order_rom = LogicVecArray(
        'ga_ls_order_rom', 'w', configs.numLdqEntries, configs.numStqEntries)
    ga_ls_order_temp = LogicVecArray(
        'ga_ls_order_temp', 'w', configs.numLdqEntries, configs.numStqEntries)
    if (configs.ldpAddrW > 0):
    ...
    arch += Mux1HROM(ga_ls_order_rom, configs.gaLdOrder,
    ...
    ldq_wen_unshifted = LogicArray(
        'ldq_wen_unshifted', 'w', configs.numLdqEntries)
    stq_wen_unshifted = LogicArray(
        'stq_wen_unshifted', 'w', configs.numStqEntries)
    ...
    for i in range(0, configs.numStqEntries):
        arch += CyclicLeftShift(ga_ls_order_temp[i],
                                ga_ls_order_rom[i], stq_tail_i)
...
def LSQ(path_rtl: str, name: str, configs: Configs):
    ...
    ldp_addr_i = LogicVecArray(
        'ldp_addr', 'i', configs.numLdPorts, configs.addrW)
    ...
    ldp_data_o = LogicVecArray(
        'ldp_data', 'o', configs.numLdPorts, configs.dataW)
    ...
    stp_addr_i = LogicVecArray(
        'stp_addr', 'i', configs.numStPorts, configs.addrW)
    ...
    stp_data_i = LogicVecArray(
        'stp_data', 'i', configs.numStPorts, configs.dataW)
    ...
    if configs.stResp:
        stp_exec_valid_o = LogicArray(
            'stp_exec_valid', 'o', configs.numStPorts)
        stp_exec_ready_i = LogicArray(
            'stp_exec_ready', 'i', configs.numStPorts)
    ...
    rreq_addr_o = LogicVecArray(
        'rreq_addr', 'o', configs.numLdMem, configs.addrW)
    ...
    rresp_data_i = LogicVecArray(
        'rresp_data', 'i', configs.numLdMem, configs.dataW)
    ...
    wreq_addr_o = LogicVecArray(
        'wreq_addr', 'o', configs.numStMem, configs.addrW)
    wreq_data_o = LogicVecArray(
        'wreq_data', 'o', configs.numStMem, configs.dataW)
    ...
    ga_ls_order = LogicVecArray(
        'ga_ls_order', 'w', configs.numLdqEntries, configs.numStqEntries)
    ...
    if configs.headLag:
        ...
        arch += CyclicPriorityMasking(ldq_head_next_oh,
    ...
    if configs.stResp:
        if configs.headLag:
            # Update the head pointer according to the valid signal of last cycle
            arch += CyclicPriorityMasking(stq_head_next_oh,
                                          stq_valid, stq_tail_oh)
        ...
        arch += Op(stq_head, stq_head_next, 'when',
    ...
    if configs.pipe0:
        ...
        load_idx_oh = LogicVecArray(
            'load_idx_oh', 'w', configs.numLdMem, configs.numStqEntries)
        ...
        bypass_idx_oh_p0 = LogicVecArray(
            'bypass_idx_oh_p0', 'r', configs.numLdqEntries, configs.numStqEntries)
        bypass_idx_oh_p0.regInit()
        bypass_en = LogicArray('bypass_en', 'w', configs.numLdqEntries)

        # Matrix Generation
        ld_st_conflict = LogicVecArray(
            'ld_st_conflict', 'w', configs.numLdqEntries, configs.numStqEntries)
        can_bypass = LogicVecArray(
            'can_bypass', 'w', configs.numLdqEntries, configs.numStqEntries)
        can_bypass_p0 = LogicVecArray(
            'can_bypass_p0', 'r', configs.numLdqEntries, configs.numStqEntries)
        can_bypass_p0.regInit(init=[0]*configs.numLdqEntries)

        if configs.pipeComp:
            ldq_valid_pcomp = LogicArray(
                'ldq_valid_pcomp', 'r', configs.numLdqEntries)
            ldq_addr_valid_pcomp = LogicArray(
                'ldq_addr_valid_pcomp', 'r', configs.numLdqEntries)
            stq_valid_pcomp = LogicArray(
                'stq_valid_pcomp', 'r', configs.numStqEntries)
            stq_addr_valid_pcomp = LogicArray(
                'stq_addr_valid_pcomp', 'r', configs.numStqEntries)
            stq_data_valid_pcomp = LogicArray(
                'stq_data_valid_pcomp', 'r', configs.numStqEntries)
            addr_valid_pcomp = LogicVecArray(
                'addr_valid_pcomp', 'w', configs.numLdqEntries, configs.numStqEntries)
            addr_same_pcomp = LogicVecArray(
                'addr_same_pcomp', 'r', configs.numLdqEntries, configs.numStqEntries)
            store_is_older_pcomp = LogicVecArray(
                'store_is_older_pcomp', 'r', configs.numLdqEntries, configs.numStqEntries)
            ...
            for i in range(0, configs.numLdqEntries):
                for j in range(0, configs.numStqEntries):
                    arch += Op((store_is_older_pcomp, i, j),
                               (store_is_older, i, j))
            for i in range(0, configs.numLdqEntries):
                for j in range(0, configs.numStqEntries):
                    arch += Op((addr_valid_pcomp, i, j), (ldq_addr_valid_pcomp,
                               i), 'and', (stq_addr_valid_pcomp, j))
            for i in range(0, configs.numLdqEntries):
                for j in range(0, configs.numStqEntries):
                    arch += Op((addr_same_pcomp, i, j), '\'1\'', 'when',
            ...
            load_conflict = LogicArray(
                'load_conflict', 'w', configs.numLdqEntries)
            load_req_valid = LogicArray(
                'load_req_valid', 'w', configs.numLdqEntries)
            ...
            for w in range(0, configs.numLdMem):
                arch += CyclicPriorityMasking(
                    load_idx_oh[w], can_load_list[w], ldq_head_oh_p0)
                arch += Reduce(load_en[w], can_load_list[w], 'or')
                if (w+1 != configs.numLdMem):
                    can_load_list.append(LogicArray(
                        f'can_load_list_{i}', 'w', configs.numLdqEntries))
                    arch += Op(can_load_list[w+1][j], 'not',
                               load_idx_oh[w], 'and', can_load_list[w][j])
            ...
            st_ld_conflict_curr = LogicVec(
                'st_ld_conflict_curr', 'w', configs.numLdqEntries)
            ...
            st_ld_conflict_next = LogicVec(
                'st_ld_conflict_next', 'w', configs.numLdqEntries)
            ...
            st_ld_conflict_p0 = LogicVec(
                'st_ld_conflict_p0', 'r', configs.numLdqEntries)
            ...
            arch += WrapAddConst(stq_issue_next, stq_issue,
            ...
            arch += MuxLookUp(store_data_valid_curr,
                              stq_data_valid_pcomp, stq_issue)
            arch += MuxLookUp(store_addr_valid_curr,
            ...
            arch += MuxLookUp(store_valid_next,
                              stq_valid_pcomp, stq_issue_next)
            arch += MuxLookUp(store_data_valid_next,
                              stq_data_valid_pcomp, stq_issue_next)
            arch += MuxLookUp(store_addr_valid_next,
            ...
            arch += Op(st_ld_conflict_p0, st_ld_conflict_next,
                       'when', stq_issue_en, 'else', st_ld_conflict_curr)
            arch += Op(can_store_p0, can_store_next, 'when',
            ...
            for i in range(0, configs.numLdqEntries):
                bypass_en_vec = LogicVec(
                    f'bypass_en_vec_{i}', 'w', configs.numStqEntries)
                # Search for the youngest store that is older than the load and conflicts
                arch += CyclicPriorityMasking(
                    bypass_idx_oh_p0[i], ld_st_conflict[i], stq_last_oh, True)
                # Check if the youngest conflict store can bypass with the load
                arch += Op(bypass_en_vec,
                           bypass_idx_oh_p0[i], 'and', can_bypass[i])
                arch += Reduce(bypass_en[i], bypass_en_vec, 'or')
        else:
            addr_valid = LogicVecArray(
                'addr_valid', 'w', configs.numLdqEntries, configs.numStqEntries)
            addr_same = LogicVecArray(
                'addr_same', 'w', configs.numLdqEntries, configs.numStqEntries)

            for i in range(0, configs.numLdqEntries):
                for j in range(0, configs.numStqEntries):
                    arch += Op((addr_valid, i, j), (ldq_addr_valid,
                               i), 'and', (stq_addr_valid, j))
            for i in range(0, configs.numLdqEntries):
                for j in range(0, configs.numStqEntries):
                    arch += Op((addr_same, i, j), '\'1\'', 'when',
                               (ldq_addr, i), '=', (stq_addr, j), 'else', '\'0\'')
        ...
        bypass_idx_oh = LogicVecArray(
            'bypass_idx_oh', 'w', configs.numLdqEntries, configs.numStqEntries)
        bypass_en = LogicArray('bypass_en', 'w', configs.numLdqEntries)
        ...
        if configs.pipeComp:
            ...
            # We do not need to check ldq_data_valid, since unissued load request cannot have valid data.
            for i in range(0, configs.numLdqEntries):
                arch += Op(load_req_valid[i], ldq_valid_pcomp[i], 'and',
                           'not', ldq_issue[i], 'and', ldq_addr_valid_pcomp[i])
            # Generate list for loads that does not face dependency issue
            for i in range(0, configs.numLdqEntries):
                arch += Op(can_load[i], 'not',
                           load_conflict[i], 'and', load_req_valid[i])
            ...
            st_ld_conflict = LogicVec(
                'st_ld_conflict', 'w', configs.numLdqEntries)
            store_conflict = Logic('store_conflict', 'w')
            ...
            arch += MuxLookUp(store_data_valid,
                              stq_data_valid_pcomp, stq_issue)
            arch += MuxLookUp(store_addr_valid,
                              stq_addr_valid_pcomp, stq_issue)
            arch += Op(store_en,
            ...
            for i in range(0, configs.numLdqEntries):
                ...
                # Search for the youngest store that is older than the load and conflicts
                arch += CyclicPriorityMasking(
                    bypass_idx_oh[i], ld_st_conflict[i], stq_last_oh, True)
                # Check if the youngest conflict store can bypass with the load
                arch += Op(bypass_en_vec,
            ...
            # We do not need to check ldq_data_valid, since unissued load request cannot have valid data.
            for i in range(0, configs.numLdqEntries):
                arch += Op(load_req_valid[i], ldq_valid[i], 'and',
    ...
    if configs.pipe1:
        ...
        load_idx_oh_p1 = LogicVecArray(
            'load_idx_oh_p1', 'r', configs.numLdMem, configs.numStqEntries)
        ...
        bypass_idx_oh_p1 = LogicVecArray(
            'bypass_idx_oh_p1', 'r', configs.numLdqEntries, configs.numStqEntries)
        ...
        for w in range(0, configs.numLdMem):
            ...
            arch += Op(load_p1_ready[w], load_hs[w],
                       'or', 'not', load_en_p1[w])
        ...
        for i in range(0, configs.numLdqEntries):
            ldq_issue_set_vec = LogicVec(
                f'ldq_issue_set_vec_{i}', 'w', configs.numLdMem)
            for w in range(0, configs.numLdMem):
        ...
        for i in range(0, configs.numLdqEntries):
            ...
            for w in range(0, configs.numLdMem):
                arch += Op(read_idx_oh[w], rresp_valid_i[w], 'when', '(',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target-dir', '-t',
                        dest='path_rtl', default='./', type=str)
    parser.add_argument('--spec-file', '-s', required=True,
                        dest='path_configs', default='', type=str)
