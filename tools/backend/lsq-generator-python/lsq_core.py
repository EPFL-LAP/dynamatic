...
class LogicVec(Logic):
    ...
    def __init__(
        self, name: str, type: str = "w", size: int = 1, init: bool = True
    ) -> None:
    ...
    def regInit(self, enable=None, init=None) -> None:
        ...
        if init != None:
            regInitString += "\t\tif (rst = '1') then\n"
            regInitString += (
                f"\t\t\t{self.getNameRead()} <= {IntToBits(init, self.size)};\n"
            )
            regInitString += "\t\telsif (rising_edge(clk)) then\n"
        else:
            regInitString += "\t\tif (rising_edge(clk)) then\n"
...
def Mux1H(dout, din, sel, j=None) -> str:
    ...
    if j == None:
        for i in range(0, length):
            str_ret += (
                "\t" * tabLevel
                + f"{mux.getNameWrite(i)} <= {din.getNameRead(i)} "
        ...
        for i in range(0, length):
            ...
            for j in range(0, mlen):
                ...
                if str_value == str_zero:
                    str_ret += (
                        "\t" * tabLevel + f"{mux.getNameWrite(j)} <= {str_zero};\n"
                    )
                else:
                    str_ret += (
                        "\t" * tabLevel
                        + f"{mux.getNameWrite(j)} <= {str_value} "
...
def PortToQueueDispatcher(
    ...
    for i in range(0, numEntries):
        arch += Op(
            entry_request_valid[i],
            entry_valid_i[i],
            "and",
            "not",
            entry_bits_valid_i[i],
        )
    ...
    for i in range(0, numEntries):
        for j in range(0, numPorts):
            arch += (
                "\t" * tabLevel + f"{entry_port_and.getNameWrite(i, j)} <= "
...
def QueueToPortDispatcher(
    ...
    arch += CyclicPriorityMasking(
        entry_port_request_prio, entry_port_request, queue_head_oh_i
    )
    ...
    for i in range(0, numEntries):
        arch += Op(
            entry_port_request_valid[i],
            entry_port_request_prio[i],
            "when",
            entry_bits_valid_i[i],
            "else",
            0,
    ...
    for i in range(0, numEntries):
        for j in range(0, numPorts):
            arch += (
                "\t" * tabLevel + f"{entry_port_hs.getNameWrite(i, j)} <= "
...
def GroupAllocator(path_rtl: str, name: str, suffix: str, configs: Configs) -> str:
    ...
    for i in range(0, configs.numGroups):
        arch += Op(
            ...
            empty_loads,
            ">=",
            (configs.gaNumLoads[i], configs.emptyLdAddrW),
            ")",
            "and",
            "(",
            empty_stores,
    ...
    if configs.gaMulti:
        ...
        for i in range(0, configs.numGroups):
            arch += Op(
                group_init_and[i], group_init_ready[i], "and", group_init_valid_i[i]
            )
        ...
        for i in range(0, configs.numGroups):
            ...
            arch += Op(
                group_init_hs[i], group_init_ready[i], "and", group_init_valid_i[i]
            )
...
def LSQ(path_rtl: str, name: str, configs: Configs):
    ...
    for i in range(0, configs.numLdqEntries):
        arch += Op(ldq_valid_next[i], "not", ldq_reset[i], "and", ldq_valid[i])
        arch += Op(ldq_valid[i], ldq_wen[i], "or", ldq_valid_next[i])
        if configs.pipe0 or configs.pipeComp:
    ...
    for i in range(0, configs.numStqEntries):
        arch += Op(stq_valid_next[i], "not", stq_reset[i], "and", stq_valid[i])
        arch += Op(stq_valid[i], stq_wen[i], "or", stq_valid_next[i])
        if configs.stResp:
            arch += Op(
                stq_exec[i],
                "not",
    ...
    arch += PortToQueueDispatcherInit(
        name + "_lda",
        configs.numLdPorts,
        configs.numLdqEntries,
        ldp_addr_i,
    ...
    arch += QueueToPortDispatcherInit(
        name + "_ldd",
        configs.numLdPorts,
        configs.numLdqEntries,
        ldp_data_o,
    ...
    if configs.pipe0:
        ...
        if configs.pipeComp:
            ...
            for i in range(0, configs.numLdqEntries):
                for j in range(0, configs.numStqEntries):
                    arch += Op(
                        ...
                        (ldq_addr_valid_pcomp, i),
                        "and",
                        (stq_addr_valid_pcomp, j),
                    )
            for i in range(0, configs.numLdqEntries):
                for j in range(0, configs.numStqEntries):
                    arch += Op(
            ...
            for i in range(0, configs.numLdqEntries):
                arch += Op(
                    can_load_p0[i], "not", load_conflict[i], "and", load_req_valid[i]
                )
            for i in range(0, configs.numLdqEntries):
            ...
            for w in range(0, configs.numLdMem):
                ...
                if w + 1 != configs.numLdMem:
                    can_load_list.append(
                        LogicArray(f"can_load_list_{i}", "w", configs.numLdqEntries)
                    )
                    arch += Op(
                        can_load_list[w + 1][j],
                        "not",
            ...
            for i in range(0, configs.numLdqEntries):
                arch += Op(
                    ...
                    "and",
                    "not",
                    MuxIndex(store_is_older_pcomp[i], stq_issue),
                    "and",
                    "(",
                    MuxIndex(addr_same_pcomp[i], stq_issue),
                    "or",
                    "not",
                    (ldq_addr_valid_pcomp, i),
                    ")",
            ...
            for i in range(0, configs.numLdqEntries):
                arch += Op(
                    ...
                    "and",
                    "not",
                    MuxIndex(store_is_older_pcomp[i], stq_issue_next),
                    "and",
                    "(",
                    MuxIndex(addr_same_pcomp[i], stq_issue_next),
                    "or",
                    "not",
                    (ldq_addr_valid_pcomp, i),
                    ")",
            ...
            arch += MuxLookUp(
                store_data_valid_next, stq_data_valid_pcomp, stq_issue_next
            )
            arch += MuxLookUp(
                store_addr_valid_next, stq_addr_valid_pcomp, stq_issue_next
            )
            arch += Op(
                can_store_next,
                store_valid_next,
            ...
            for i in range(0, configs.numLdqEntries):
                arch += Op(
                    ...
                    "and",
                    "not",
                    MuxIndex(store_is_older[i], stq_issue),
                    "and",
                    "(",
                    MuxIndex(addr_same[i], stq_issue),
                    "or",
                    "not",
                    (ldq_addr_valid, i),
                    ")",
            ...
            for i in range(0, configs.numLdqEntries):
                arch += Op(
                    ...
                    "and",
                    "not",
                    MuxIndex(store_is_older[i], stq_issue_next),
                    "and",
                    "(",
                    MuxIndex(addr_same[i], stq_issue_next),
                    "or",
                    "not",
                    (ldq_addr_valid, i),
                    ")",
        ...
        if configs.pipeComp:
            ...
            for i in range(0, configs.numLdqEntries):
                arch += Op(
                    can_load[i], "not", load_conflict[i], "and", load_req_valid[i]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--spec-file", "-s", required=True, dest="path_configs", default="", type=str
    )
