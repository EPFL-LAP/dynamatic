...
def WrapAdd(out, in_a, in_b, max: int) -> str:
    ...
    str_ret += (
        "\t" * tabLevel + f"-- WrapAdd({out.name}, {in_a.name}, {in_b.name}, {max})\n"
    )
    if isPow2(max):
        str_ret += (
            "\t" * tabLevel
...
def WrapAddConst(out, in_a, const: int, max: int) -> str:
    ...
    str_ret = "\t" * tabLevel + "-- WrapAdd Begin\n"
    str_ret += (
        "\t" * tabLevel + f"-- WrapAdd({out.name}, {in_a.name}, {const}, {max})\n"
    )
    if isPow2(max):
        str_ret += (
            "\t" * tabLevel
...
def WrapSub(out, in_a, in_b, max: int) -> str:
    ...
    str_ret += (
        "\t" * tabLevel + f"-- WrapSub({out.name}, {in_a.name}, {in_b.name}, {max})\n"
    )
    if isPow2(max):
        str_ret += (
            "\t" * tabLevel
...
def PortToQueueDispatcherInit(
    ...
    for i in range(0, numPorts):
        arch += (
            "\t" * tabLevel + f"port_ready_{i}_o => {port_ready_o.getNameWrite(i)},\n"
        )
    for i in range(0, numPorts):
        arch += (
            "\t" * tabLevel + f"port_valid_{i}_i => {port_valid_i.getNameRead(i)},\n"
        )
    for i in range(0, numEntries):
        arch += (
            "\t" * tabLevel + f"entry_valid_{i}_i => {entry_valid_i.getNameRead(i)},\n"
        )
    for i in range(0, numEntries):
        arch += (
            "\t" * tabLevel
    ...
    for i in range(0, numEntries):
        arch += (
            "\t" * tabLevel + f"entry_bits_{i}_o => {entry_bits_o.getNameWrite(i)},\n"
        )
    for i in range(0, numEntries):
...
def QueueToPortDispatcherInit(
    ...
    for i in range(0, numPorts):
        if port_bits_o != None:
            arch += (
                "\t" * tabLevel + f"port_bits_{i}_o => {port_bits_o.getNameWrite(i)},\n"
            )
    for i in range(0, numPorts):
        arch += (
            "\t" * tabLevel + f"port_ready_{i}_i => {port_ready_i.getNameRead(i)},\n"
        )
    for i in range(0, numPorts):
        arch += (
            "\t" * tabLevel + f"port_valid_{i}_o => {port_valid_o.getNameWrite(i)},\n"
        )
    for i in range(0, numEntries):
        arch += (
            "\t" * tabLevel + f"entry_valid_{i}_i => {entry_valid_i.getNameRead(i)},\n"
    ...
    for i in range(0, numEntries):
        arch += (
            "\t" * tabLevel + f"entry_reset_{i}_o => {entry_reset_o.getNameWrite(i)},\n"
        )
    arch += "\t" * tabLevel + f"queue_head_oh_i => {queue_head_oh_i.getNameRead()}\n"
    tabLevel -= 1
    arch += "\t" * tabLevel + f");\n"
...
def GroupAllocatorInit(
    ...
    for i in range(0, configs.numLdqEntries):
        arch += (
            "\t" * tabLevel + f"ga_ls_order_{i}_o => {ga_ls_order_o.getNameWrite(i)},\n"
        )
