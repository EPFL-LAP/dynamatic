...
class LSQ:
    ...
    def generate(self, em: Emitter, lsq_submodules, path_rtl) -> None:
        ...
        rreq_valid_o = LogicArray(
            em, 'rreq_valid', 'o', self.configs.numLdMem)
        rreq_ready_i = LogicArray(
            em, 'rreq_ready', 'i', self.configs.numLdMem)
        ...
        rresp_valid_i = LogicArray(
            em, 'rresp_valid', 'i', self.configs.numLdMem)
        rresp_ready_o = LogicArray(
            em, 'rresp_ready', 'o', self.configs.numLdMem)
        ...
        wreq_valid_o = LogicArray(
            em, 'wreq_valid', 'o', self.configs.numStMem)
        wreq_ready_i = LogicArray(
            em, 'wreq_ready', 'i', self.configs.numStMem)
        ...
        wresp_valid_i = LogicArray(
            em, 'wresp_valid', 'i', self.configs.numStMem)
        wresp_ready_o = LogicArray(
            em, 'wresp_ready', 'o', self.configs.numStMem)
        ...
        if (self.configs.master):
            ...
            ldq_head_oh = LogicVec(em, 'ldq_head_oh', 'w',
                                   self.configs.numLdqEntries)
            stq_head_oh = LogicVec(em, 'stq_head_oh', 'w',
                                   self.configs.numStqEntries)

            #! The memory completion signal cannot be set to 1 when any group is allocating:
            binops = [Bin(Val(group_init_valid_i, i), BinOp.OR, None) for i in range(group_init_valid_i.length - 1)]
            ...
            em.add_assignment(temp_gen_mem, ctrlEnd_valid & stq_empty & ldq_empty & no_curr_ga)
...
always @(posedge clk) begin
    if (rst) begin
        ...
        ldq_addr_wen = LogicArray(
            em, 'ldq_addr_wen', 'w', self.configs.numLdqEntries)
        ldq_reset = LogicArray(em, 'ldq_reset', 'w',
                               self.configs.numLdqEntries)
        stq_wen = LogicArray(em, 'stq_wen', 'w', self.configs.numStqEntries)
        stq_addr_wen = LogicArray(
            em, 'stq_addr_wen', 'w', self.configs.numStqEntries)
        stq_data_wen = LogicArray(
            em, 'stq_data_wen', 'w', self.configs.numStqEntries)
        stq_reset = LogicArray(em, 'stq_reset', 'w',
                               self.configs.numStqEntries)
        # From Read/Write Block
        ldq_data_wen = LogicArray(
            em, 'ldq_data_wen', 'w', self.configs.numLdqEntries)
        ldq_issue_set = LogicArray(
            em, 'ldq_issue_set', 'w', self.configs.numLdqEntries)
        if self.configs.stResp:
        ...
        if self.configs.pipe0 or self.configs.pipeComp:
            ldq_wen_p0 = LogicArray(
                em, 'ldq_wen_p0', 'r', self.configs.numLdqEntries)
            ldq_wen_p0.regInit()
        ...
        for i in range(0, self.configs.numLdqEntries):
            ...
            if self.configs.pipe0 or self.configs.pipeComp:
                ...
                if self.configs.pipe0 and self.configs.pipeComp:
                    ...
                    em.add_assignment(ldq_issue[i], ~ldq_wen_p1[i] & (ldq_issue_set[i] | ldq_issue[i]))
                else:
                    em.add_assignment(ldq_issue[i], ~ldq_wen_p0[i] & (ldq_issue_set[i] | ldq_issue[i]))
            else:
                em.add_assignment(ldq_issue[i], ~ldq_wen[i] & (ldq_issue_set[i] | ldq_issue[i]))

            em.add_assignment(ldq_addr_valid[i], ~ldq_wen[i] & (ldq_addr_wen[i] | ldq_addr_valid[i]))
            em.add_assignment(ldq_data_valid[i], ~ldq_wen[i] & (ldq_data_wen[i] | ldq_data_valid[i]))
        # store queue
        ...
        for i in range(0, self.configs.numStqEntries):
            ...
            if self.configs.stResp:
                em.add_assignment(stq_exec[i], ~stq_wen[i] & (stq_exec_set[i] | stq_exec[i]))
            em.add_assignment(stq_addr_valid[i], ~stq_wen[i] & (stq_addr_wen[i] | stq_addr_valid[i]))
            em.add_assignment(stq_data_valid[i], ~stq_wen[i] & (stq_data_wen[i] | stq_data_valid[i]))
        ...
        for i in range(0, self.configs.numLdqEntries):
            for j in range(0, self.configs.numStqEntries):
                em.add_assignment((store_is_older, i, j), (~Val(stq_reset, j) & (Val(stq_alloc, j) | Val(ga_ls_order, i, j)))
                                                            .when(Val(ldq_wen, i))
        ...
        WrapAdd(em, ldq_tail, ldq_tail, num_loads,
                        self.configs.numLdqEntries)
        WrapAdd(em, stq_tail, stq_tail, num_stores,
                       self.configs.numStqEntries)
        WrapAddConst(em, stq_issue, stq_issue, 1,
                             self.configs.numStqEntries)
        WrapAddConst(em, stq_resp, stq_resp, 1,
                             self.configs.numStqEntries)

        ldq_tail_oh = LogicVec(em, 'ldq_tail_oh', 'w',
                               self.configs.numLdqEntries)
        BitsToOH(em, ldq_tail_oh, ldq_tail)
        ldq_head_next_oh = LogicVec(
            em, 'ldq_head_next_oh', 'w', self.configs.numLdqEntries)
        ldq_head_next = LogicVec(em, 'ldq_head_next',
        ...
        if self.configs.stResp:
            if self.configs.headLag:
                ...
                CyclicPriorityMasking(em, stq_head_next_oh,
                                      stq_alloc_next, stq_tail_oh)
                Reduce(em, stq_head_sel, stq_alloc_next, BinOp.OR)
            OHToBits(em, stq_head_next, stq_head_next_oh)
            em.add_assignment(stq_head, stq_head_next.when(stq_head_sel).else_(stq_tail))
        else:
            WrapAddConst(em, stq_head_next, stq_head,
                         1, self.configs.numStqEntries)
            em.add_assignment(stq_head_sel, wresp_valid_i[0])
            em.add_assignment(stq_head, stq_head_next.when(stq_head_sel).else_(stq_head))
        ...
        if self.configs.pipe0:
            ...
            bypass_idx_oh_p0 = LogicVecArray(
                em, 'bypass_idx_oh_p0', 'r', self.configs.numLdqEntries, self.configs.numStqEntries)
            bypass_idx_oh_p0.regInit()
            bypass_en = LogicArray(em, 'bypass_en', 'w',
                                   self.configs.numLdqEntries)
            ...
            can_bypass = LogicVecArray(
                em, 'can_bypass', 'w', self.configs.numLdqEntries, self.configs.numStqEntries)
            can_bypass_p0 = LogicVecArray(
                em, 'can_bypass_p0', 'r', self.configs.numLdqEntries, self.configs.numStqEntries)
            ...
            if self.configs.pipeComp:
                ...
                addr_valid_pcomp = LogicVecArray(
                    em, 'addr_valid_pcomp', 'w', self.configs.numLdqEntries, self.configs.numStqEntries)
                addr_same_pcomp = LogicVecArray(
                    em, 'addr_same_pcomp', 'r', self.configs.numLdqEntries, self.configs.numStqEntries)
                store_is_older_pcomp = LogicVecArray(
                    em, 'store_is_older_pcomp', 'r', self.configs.numLdqEntries, self.configs.numStqEntries)
                ...
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em.add_assignment((store_is_older_pcomp, i, j), Val(store_is_older, i, j))
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em.add_assignment((addr_valid_pcomp, i, j), Val(ldq_addr_valid_pcomp, i) & Val(stq_addr_valid_pcomp, j))
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em.add_assignment((addr_same_pcomp, i, j), Bit(1).when(Val(ldq_addr, i) == Val(stq_addr, j)).else_(Bit(0)))
                ...
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em.add_assignment((ld_st_conflict, i, j), Val(stq_alloc_pcomp, j) 
                ...
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em.add_assignment((can_bypass_p0, i, j), Val(ldq_alloc_pcomp, i) 
                                                                & Val(stq_data_valid_pcomp, j) 
                                                                & Val(addr_same_pcomp, i, j) 
                                                                & Val(addr_valid_pcomp, i, j))
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em.add_assignment((can_bypass, i, j), ~Val(ldq_issue, i) & Val(can_bypass_p0, i, j))
                ...
                can_load = LogicArray(
                    em, 'can_load', 'w', self.configs.numLdqEntries)
                ...
                for i in range(0, self.configs.numLdqEntries):
                    em.add_assignment(load_req_valid[i], ldq_alloc_pcomp[i] & ldq_addr_valid_pcomp[i])
                ...
                for w in range(0, self.configs.numLdMem):
                    ...
                    if (w+1 != self.configs.numLdMem):
                        ...
                        for i in range(0, self.configs.numLdqEntries):
                            em.add_assignment(can_load_list[w+1][i], ~load_idx_oh_LogicArray[i] & can_load_list[w][i])
                ...
                store_data_valid_curr = Logic(
                    em, 'store_data_valid_curr', 'w')
                store_addr_valid_curr = Logic(
                    em, 'store_addr_valid_curr', 'w')
                ...
                store_data_valid_next = Logic(
                    em, 'store_data_valid_next', 'w')
                store_addr_valid_next = Logic(
                    em, 'store_addr_valid_next', 'w')
                ...
                WrapAddConst(em, stq_issue_next,
                ...
                for i in range(0, self.configs.numLdqEntries):
                    em.add_assignment((st_ld_conflict_curr, i),
                                Val(ldq_alloc_pcomp, i) 
                                & ~Val(em.mux_index(store_is_older_pcomp[i], stq_issue))
                                & (Val(em.mux_index(addr_same_pcomp[i], stq_issue)) | ~Val(ldq_addr_valid_pcomp, i))
                                )
                for i in range(0, self.configs.numLdqEntries):
                    em.add_assignment((st_ld_conflict_next, i),
                               Val(ldq_alloc_pcomp, i)
                               & ~Val(em.mux_index(store_is_older_pcomp[i], stq_issue_next))
                               & (Val(em.mux_index(addr_same_pcomp[i], stq_issue_next)) | ~Val(ldq_addr_valid_pcomp, i))
                               )
                # The store is valid whe the entry is valid and the data is also valid,
                # the store address should also be valid
                MuxLookUp(em, store_valid_curr,
                                  stq_alloc_pcomp, stq_issue)
                MuxLookUp(em, store_data_valid_curr,
                                  stq_data_valid_pcomp, stq_issue)
                MuxLookUp(em, store_addr_valid_curr,
                                  stq_addr_valid_pcomp, stq_issue)
                em.add_assignment(can_store_curr,
                           store_valid_curr & store_data_valid_curr & store_addr_valid_curr)
                MuxLookUp(em, store_valid_next,
                          stq_alloc_pcomp, stq_issue_next)
                MuxLookUp(em, store_data_valid_next,
                          stq_data_valid_pcomp, stq_issue_next)
                MuxLookUp(em, store_addr_valid_next,
                                  stq_addr_valid_pcomp, stq_issue_next)
                em.add_assignment(can_store_next,
                           store_valid_next & store_data_valid_next & store_addr_valid_next)
                # Multiplex from current and next
                em.add_assignment(st_ld_conflict_p0, st_ld_conflict_next.when(stq_issue_en).else_(st_ld_conflict_curr))
                em.add_assignment(can_store_p0, can_store_next.when(stq_issue_en).else_(can_store_curr))
                ...
                for i in range(0, self.configs.numLdqEntries):
                    ...
                    CyclicPriorityMasking(em, bypass_idx_oh_p0[i], ld_st_conflict[i], stq_last_oh, True)
                ...
                addr_valid = LogicVecArray(
                    em, 'addr_valid', 'w', self.configs.numLdqEntries, self.configs.numStqEntries)
                addr_same = LogicVecArray(
                    em, 'addr_same', 'w', self.configs.numLdqEntries, self.configs.numStqEntries)

                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em_add_assignment((addr_valid, i, j), Val(ldq_addr_valid, i) & Val(stq_addr_valid, j))
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em_add_assignment((addr_same, i, j), Bit(1).when(Val(ldq_addr, i) == Val(stq_addr, j)).else_(Bit(0)))
                ...
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em.add_assignment((ld_st_conflict, i, j),
                                   Val(stq_alloc, j) 
                ...
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em.add_assignment((can_bypass_p0, i, j),
                                       Val(ldq_alloc, i) 
                ...
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em.add_assignment((can_bypass, i, j),
                ...
                # We do not need to check ldq_data_valid, since unissued load request cannot have valid data.
                for i in range(0, self.configs.numLdqEntries):
                    em.add_assignment(load_req_valid[i], ldq_alloc[i] & ldq_addr_valid[i])
                # Generate list for loads that does not face dependency issue
                for i in range(0, self.configs.numLdqEntries):
                    em.add_assignment(can_load_p0[i], ~load_conflict[i] & load_req_valid[i])
                for i in range(0, self.configs.numLdqEntries):
                ...
                for i in range(0, self.configs.numLdqEntries):
                    em.add_assignment((st_ld_conflict_curr, i),
                               ...
                               & (Val(em.mux_index(addr_same[i], stq_issue)) | ~Val(ldq_addr_valid, i))
                               )
                for i in range(0, self.configs.numLdqEntries):
                    em.add_assignment((st_ld_conflict_next, i),
                               Val(ldq_alloc, i)
                               & ~Val(em.mux_index(store_is_older[i], stq_issue_next))
                               & (Val(em.mux_index(addr_same[i], stq_issue_next)) | ~Val(ldq_addr_valid, i))
                               )
                # The store is valid whe the entry is valid and the data is also valid,
                # the store address should also be valid
            ...
            bypass_idx_oh = LogicVecArray(
                em, 'bypass_idx_oh', 'w', self.configs.numLdqEntries, self.configs.numStqEntries)
            bypass_en = LogicArray(em, 'bypass_en', 'w',
                                   self.configs.numLdqEntries)
            ...
            if self.configs.pipeComp:
                ...
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em.add_assignment((addr_same_pcomp, i, j), Bit(1)
                ...
                for i in range(0, self.configs.numLdqEntries):
                    Reduce(em,
                                   load_conflict[i], ld_st_conflict[i], BinOp.OR)
                # The load is valid when the entry is valid and not yet issued, the load address should also be valid.
                # We do not need to check ldq_data_valid, since unissued load request cannot have valid data.
                for i in range(0, self.configs.numLdqEntries):
                    em.add_assignment(load_req_valid[i], ldq_alloc_pcomp[i] & ~ldq_issue[i] & ldq_addr_valid_pcomp[i])
                # Generate list for loads that does not face dependency issue
                for i in range(0, self.configs.numLdqEntries):
                    em.add_assignment(can_load[i], ~load_conflict[i] & load_req_valid[i])

                can_load_list = []
                can_load_list.append(can_load)
                for w in range(0, self.configs.numLdMem):
                    CyclicPriorityMasking(em, load_idx_oh[w], can_load_list[w], ldq_head_oh)
                    Reduce(em, load_en[w], can_load_list[w], BinOp.OR)
                    if (w+1 != self.configs.numLdMem):
                        load_idx_oh_LogicArray = LogicArray(
                            em, f'load_idx_oh_Array_{w+1}', 'w', self.configs.numLdqEntries)
                ...
                # Index order are reversed for store matrix.
                for i in range(0, self.configs.numLdqEntries):
                    em.add_assignment((st_ld_conflict, i),
                               Val(ldq_alloc_pcomp, i) 
                               & ~Val(em.mux_index(store_is_older_pcomp[i], stq_issue))
                               & (Val(em.mux_index(addr_same_pcomp[i], stq_issue)) | ~Val(ldq_addr_valid_pcomp, i))
                               )
                ...
                MuxLookUp(em, store_data_valid,
                                  stq_data_valid_pcomp, stq_issue)
                MuxLookUp(em, store_addr_valid,
                                  stq_addr_valid_pcomp, stq_issue)
                em.add_assignment(store_en,
                           ~store_conflict
                           & store_valid
                           & store_data_valid
                           & store_addr_valid
                ...
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em.add_assignment((addr_valid, i, j), Val(ldq_addr_valid, i) & Val(stq_addr_valid, j))
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                        em.add_assignment((addr_same, i, j), 
                ...
                # We do not need to check ldq_data_valid, since unissued load request cannot have valid data.
                for i in range(0, self.configs.numLdqEntries):
                    em.add_assignment(load_req_valid[i], ldq_alloc[i] & ~ldq_issue[i] & ldq_addr_valid[i])
                # Generate list for loads that does not face dependency issue
                for i in range(0, self.configs.numLdqEntries):
                    em.add_assignment(can_load[i], ~load_conflict[i] & load_req_valid[i])
        ...
        if self.configs.pipe1:
            ...
            load_idx_oh_p1 = LogicVecArray(
                em, 'load_idx_oh_p1', 'r', self.configs.numLdMem, self.configs.numLdqEntries)
            load_en_p1 = LogicArray(
                em, 'load_en_p1', 'r', self.configs.numLdMem)

            load_hs = LogicArray(em, 'load_hs', 'w', self.configs.numLdMem)
            load_p1_ready = LogicArray(
                em, 'load_p1_ready', 'w', self.configs.numLdMem)

            store_idx_p1 = LogicVec(
                em, 'store_idx_p1', 'r', self.configs.stqAddrW)
            ...
            bypass_idx_oh_p1 = LogicVecArray(
                em, 'bypass_idx_oh_p1', 'r', self.configs.numLdqEntries, self.configs.numStqEntries)
            ...
            for i in range(0, self.configs.numLdqEntries):
                ...
                for w in range(0, self.configs.numLdMem):
                    em.add_assignment((ldq_issue_set_vec, w),
                               (Val(load_idx_oh, w, i) & Val(load_p1_ready, w)) | Val(bypass_en, i)
                               )
            ...
            for i in range(0, self.configs.numLdqEntries):
                ...
                read_data = LogicVec(
                    em, f'read_data_{i}', 'w', self.configs.dataW)
                for w in range(0, self.configs.numLdMem):
                    em.add_assignment(read_idx_oh[w], rresp_valid_i[w].when((rresp_id_i[w] == Val(i, self.configs.idW)).else_(Bit(0))))
                ...
                bypass_data = LogicVec(
                    em, f'bypass_data_{i}', 'w', self.configs.dataW)
            ...
            if self.configs.stResp:
                for i in range(0, self.configs.numStqEntries):
                    em.add_assignment(stq_exec_set[i], wresp_valid_i[0].when((stq_resp == Val(i, self.configs.stqAddrW))).else_(Bit(0)))
            else:
                for i in range(0, self.configs.numStqEntries):
                    em.add_assignment(stq_reset[i],
                               wresp_valid_i[0].when((stq_resp == Val(i, self.configs.stqAddrW))).else_(Bit(0)))
            ...
            for i in range(0, self.configs.numLdqEntries):
                ...
                for w in range(0, self.configs.numLdMem):
                    em.add_assignment((ldq_issue_set_vec, w),
                               (Val(load_idx_oh, w, i) & Val(rreq_ready_i, w) & Val(load_en, w))
                               | Val(bypass_en, i)
                               )
                Reduce(em, ldq_issue_set[i], ldq_issue_set_vec, BinOp.OR)
            ...
            if self.configs.stResp:
                ...
                for i in range(0, self.configs.numStqEntries):
                    em.add_assignment(stq_reset[i], wresp_valid_i[0].when(stq_resp == Val(i, self.configs.stqAddrW)).else_(Bit(0)))
            em.add_assignment(stq_resp_en, wresp_valid_i[0])
            em.add_assignment(wresp_ready_o[0], Bit(1))
