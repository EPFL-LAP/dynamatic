...
def MASKED_NUM_NEW_ENTRIES_NAME(
        queue_type : QueueType, 
        ):
    """
    RTL name for the "number of new (load/store) queue entries" signal,
    specific to each group and masked by its valid signal
    """
    return f"{NUM_NEW_ENTRIES_NAME(queue_type)}_masked"
...
This singular signal has 1 bit per group of memory operations.
"""

# GROUP_INIT_TRANSFER_NAME = f"{GROUP_INIT_CHANNEL_NAME}_hs"
# """
# RTL name for signals which identify whether a 
# particular group init channel is 'transfer' in this cycle.
