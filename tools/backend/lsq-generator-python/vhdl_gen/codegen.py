...
def codeGen(path_rtl, configs):
    ...
    ptq_dispatcher_lda = dispatchers.PortToQueueDispatcher(
        ctx, path_rtl, name, '_lda', configs.numLdPorts, configs.numLdqEntries, configs.addrW, configs.ldpAddrW)
    ...
    qtp_dispatcher_ldd = dispatchers.QueueToPortDispatcher(
        ctx, path_rtl, name, '_ldd', configs.numLdPorts, configs.numLdqEntries, configs.dataW, configs.ldpAddrW)
    ...
    ptq_dispatcher_sta = dispatchers.PortToQueueDispatcher(
        ctx, path_rtl, name, '_sta', configs.numStPorts, configs.numStqEntries, configs.addrW, configs.stpAddrW)
    ...
    ptq_dispatcher_std = dispatchers.PortToQueueDispatcher(
        ctx, path_rtl, name, '_std', configs.numStPorts, configs.numStqEntries, configs.dataW, configs.stpAddrW)
    ...
    if configs.stResp:
        qtp_dispatcher_stb = dispatchers.QueueToPortDispatcher(
            ctx, path_rtl, name, '_stb', configs.numStPorts, configs.numStqEntries, 0, configs.stpAddrW)
