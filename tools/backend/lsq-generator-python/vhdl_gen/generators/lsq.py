    ...
    def generate(self) -> None:
        ...
        *Instantiation of LSQ is in lsq-generator.py.
        ...
        rreq_valid_o = LogicArray(
            self.ctx, 'rreq_valid', 'o', self.configs.numLdMem)
        rreq_ready_i = LogicArray(
            self.ctx, 'rreq_ready', 'i', self.configs.numLdMem)
        ...
        rresp_valid_i = LogicArray(
            self.ctx, 'rresp_valid', 'i', self.configs.numLdMem)
        rresp_ready_o = LogicArray(
            self.ctx, 'rresp_ready', 'o', self.configs.numLdMem)
        ...
        wreq_valid_o = LogicArray(
            self.ctx, 'wreq_valid', 'o', self.configs.numStMem)
        wreq_ready_i = LogicArray(
            self.ctx, 'wreq_ready', 'i', self.configs.numStMem)
        ...
        wresp_valid_i = LogicArray(
            self.ctx, 'wresp_valid', 'i', self.configs.numStMem)
        wresp_ready_o = LogicArray(
            self.ctx, 'wresp_ready', 'o', self.configs.numStMem)
        ...
        ######  Queue Registers ######
        # Load Queue Entries
        ldq_valid = LogicArray(self.ctx, 'ldq_valid', 'r',
                               self.configs.numLdqEntries)
        ldq_issue = LogicArray(self.ctx, 'ldq_issue', 'r',
                               self.configs.numLdqEntries)
        ...
        # Store Queue Entries
        stq_valid = LogicArray(self.ctx, 'stq_valid', 'r',
                               self.configs.numStqEntries)
        if self.configs.stResp:
            stq_exec = LogicArray(self.ctx, 'stq_exec',
                                  'r', self.configs.numStqEntries)
        ...
        ldq_wen = LogicArray(self.ctx, 'ldq_wen', 'w',
                             self.configs.numLdqEntries)
        ldq_addr_wen = LogicArray(
            self.ctx, 'ldq_addr_wen', 'w', self.configs.numLdqEntries)
        ldq_reset = LogicArray(self.ctx, 'ldq_reset', 'w',
                               self.configs.numLdqEntries)
        stq_wen = LogicArray(self.ctx, 'stq_wen', 'w',
                             self.configs.numStqEntries)
        stq_addr_wen = LogicArray(
            self.ctx, 'stq_addr_wen', 'w', self.configs.numStqEntries)
        stq_data_wen = LogicArray(
            self.ctx, 'stq_data_wen', 'w', self.configs.numStqEntries)
        stq_reset = LogicArray(self.ctx, 'stq_reset', 'w',
                               self.configs.numStqEntries)
        # From Read/Write Block
        ldq_data_wen = LogicArray(
            self.ctx, 'ldq_data_wen', 'w', self.configs.numLdqEntries)
        ...
        ldq_head_oh = LogicVec(self.ctx, 'ldq_head_oh',
                               'w', self.configs.numLdqEntries)
        stq_head_oh = LogicVec(self.ctx, 'stq_head_oh',
        ...
        if self.configs.pipe0 or self.configs.pipeComp:
            ldq_wen_p0 = LogicArray(
                self.ctx, 'ldq_wen_p0', 'r', self.configs.numLdqEntries)
            ldq_wen_p0.regInit()
        ...
        arch += WrapAdd(self.ctx, ldq_tail, ldq_tail,
                        num_loads, self.configs.numLdqEntries)
        arch += WrapAdd(self.ctx, stq_tail, stq_tail,
                        num_stores, self.configs.numStqEntries)
        arch += WrapAddConst(self.ctx, stq_issue, stq_issue,
                             1, self.configs.numStqEntries)
        arch += WrapAddConst(self.ctx, stq_resp, stq_resp,
                             1, self.configs.numStqEntries)

        ldq_tail_oh = LogicVec(self.ctx, 'ldq_tail_oh',
                               'w', self.configs.numLdqEntries)
        ...
        ldq_head_next = LogicVec(
            self.ctx, 'ldq_head_next', 'w', self.configs.ldqAddrW)
        ...
        stq_tail_oh = LogicVec(self.ctx, 'stq_tail_oh',
                               'w', self.configs.numStqEntries)
        ...
        stq_head_next = LogicVec(
            self.ctx, 'stq_head_next', 'w', self.configs.stqAddrW)
        ...
        if self.configs.pipe0:
            ...
            bypass_en = LogicArray(self.ctx, 'bypass_en',
                                   'w', self.configs.numLdqEntries)
            ...
            if self.configs.pipeComp:
                ...
                for i in range(0, self.configs.numLdqEntries):
                    ...
                    arch += Op(self.ctx, (ldq_addr_valid_pcomp, i),
                               (ldq_addr_valid, i))
                for j in range(0, self.configs.numStqEntries):
                    arch += Op(self.ctx, (stq_valid_pcomp, j), (stq_valid, j))
                    arch += Op(self.ctx, (stq_addr_valid_pcomp, j),
                               (stq_addr_valid, j))
                    arch += Op(self.ctx, (stq_data_valid_pcomp, j),
                               (stq_data_valid, j))
                for i in range(0, self.configs.numLdqEntries):
                    for j in range(0, self.configs.numStqEntries):
                ...
                can_load = LogicArray(
                    self.ctx, 'can_load', 'w', self.configs.numLdqEntries)
                ...
                store_data_valid_curr = Logic(
                    self.ctx, 'store_data_valid_curr', 'w')
                store_addr_valid_curr = Logic(
                    self.ctx, 'store_addr_valid_curr', 'w')
                ...
                store_data_valid_next = Logic(
                    self.ctx, 'store_data_valid_next', 'w')
                store_addr_valid_next = Logic(
                    self.ctx, 'store_addr_valid_next', 'w')
                ...
                arch += Reduce(self.ctx, store_conflict,
                               st_ld_conflict_p0, 'or')
            ...
            if self.configs.pipeComp:
                ...
                # The store is valid whe the entry is valid and the data is also valid,
                # the store address should also be valid
                arch += MuxLookUp(self.ctx, store_valid,
                                  stq_valid_pcomp, stq_issue)
        ...
        if self.configs.pipe1:
            ...
            load_en_p1 = LogicArray(
                self.ctx, 'load_en_p1', 'r', self.configs.numLdMem)

            load_hs = LogicArray(self.ctx, 'load_hs', 'w',
                                 self.configs.numLdMem)
            load_p1_ready = LogicArray(
                self.ctx, 'load_p1_ready', 'w', self.configs.numLdMem)

            store_idx_p1 = LogicVec(
                self.ctx, 'store_idx_p1', 'r', self.configs.stqAddrW)
            ...
            for w in range(0, self.configs.numLdMem):
                arch += Op(self.ctx, load_hs[w],
                           load_en_p1[w], 'and', rreq_ready_i[w])
            ...
            arch += Op(self.ctx, store_p1_ready,
                       store_hs, 'or', 'not', store_en_p1)
            ...
            if self.configs.pipe0:
                for i in range(0, self.configs.numLdqEntries):
                    arch += Op(self.ctx,
                               bypass_idx_oh_p1[i], bypass_idx_oh_p0[i])
            else:
                for i in range(0, self.configs.numLdqEntries):
            ...
            for w in range(0, self.configs.numLdMem):
                ...
                arch += Mux1H(self.ctx,
                              rreq_addr_o[w], ldq_addr, load_idx_oh_p1[w])
            ...
            for i in range(0, self.configs.numLdqEntries):
                ...
                arch += Reduce(self.ctx,
                               ldq_issue_set[i], ldq_issue_set_vec, 'or')
            ...
            for i in range(0, self.configs.numLdqEntries):
                ...
                read_data = LogicVec(
                    self.ctx, f'read_data_{i}', 'w', self.configs.dataW)
                for w in range(0, self.configs.numLdMem):
                ...
                bypass_data = LogicVec(
                    self.ctx, f'bypass_data_{i}', 'w', self.configs.dataW)
                arch += Mux1H(self.ctx, bypass_data,
                              stq_data, bypass_idx_oh_p1[i])
                # multiplex from read and bypass data
                arch += Op(self.ctx, ldq_data[i], read_data, 'or', bypass_data)
                arch += Op(self.ctx, ldq_data_wen[i],
            ...
            for w in range(0, self.configs.numLdMem):
                ...
                arch += Mux1H(self.ctx,
                              rreq_addr_o[w], ldq_addr, load_idx_oh[w])
            ...
            arch += Op(self.ctx, stq_issue_en,
                       store_en, 'and', wreq_ready_i[0])
            ...
            for i in range(0, self.configs.numLdqEntries):
                ...
                if self.configs.pipe0:
                    arch += Mux1H(self.ctx, bypass_data,
                                  stq_data, bypass_idx_oh_p0[i])
                else:
                    arch += Mux1H(self.ctx, bypass_data,
                                  stq_data, bypass_idx_oh[i])
                # multiplex from read and bypass data
                arch += Op(self.ctx, ldq_data[i], read_data, 'or', bypass_data)
                arch += Op(self.ctx, ldq_data_wen[i],
