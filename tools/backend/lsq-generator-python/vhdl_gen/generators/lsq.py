...
class LSQ:
    ...
    def generate(self, gen_container, path_rtl) -> None:
        ...
        *Instantiation of LSQ is in lsq-generator.py.
        ...
        rreq_valid_o = LogicArray(ctx, 'rreq_valid', 'o', self.configs.numLdMem)
        rreq_ready_i = LogicArray(ctx, 'rreq_ready', 'i', self.configs.numLdMem)
        ...
        rresp_valid_i = LogicArray(ctx, 'rresp_valid', 'i', self.configs.numLdMem)
        rresp_ready_o = LogicArray(ctx, 'rresp_ready', 'o', self.configs.numLdMem)
        ...
        wreq_valid_o = LogicArray(ctx, 'wreq_valid', 'o', self.configs.numStMem)
        wreq_ready_i = LogicArray(ctx, 'wreq_ready', 'i', self.configs.numStMem)
        ...
        wresp_valid_i = LogicArray(ctx, 'wresp_valid', 'i', self.configs.numStMem)
        wresp_ready_o = LogicArray(ctx, 'wresp_ready', 'o', self.configs.numStMem)
        ...
        ######  Queue Registers ######
        # Load Queue Entries
        ldq_valid = LogicArray(ctx, 'ldq_valid', 'r', self.configs.numLdqEntries)
        ldq_issue = LogicArray(ctx, 'ldq_issue', 'r', self.configs.numLdqEntries)
        ...
        # Store Queue Entries
        stq_valid = LogicArray(ctx, 'stq_valid', 'r', self.configs.numStqEntries)
        if self.configs.stResp:
            stq_exec = LogicArray(ctx, 'stq_exec', 'r', self.configs.numStqEntries)
        ...
        ldq_addr_wen = LogicArray(ctx, 'ldq_addr_wen', 'w', self.configs.numLdqEntries)
        ldq_reset = LogicArray(ctx, 'ldq_reset', 'w', self.configs.numLdqEntries)
        ...
        ldq_data_wen = LogicArray(ctx, 'ldq_data_wen', 'w', self.configs.numLdqEntries)
        ...
        ldq_head_oh = LogicVec(ctx, 'ldq_head_oh', 'w', self.configs.numLdqEntries)
        stq_head_oh = LogicVec(ctx, 'stq_head_oh', 'w', self.configs.numStqEntries)
        ...
        if self.configs.pipe0 or self.configs.pipeComp:
            ldq_wen_p0 = LogicArray(ctx, 'ldq_wen_p0', 'r', self.configs.numLdqEntries)
            ldq_wen_p0.regInit()
        ...
        arch += WrapAdd(ctx, ldq_tail, ldq_tail, num_loads, self.configs.numLdqEntries)
        arch += WrapAdd(ctx, stq_tail, stq_tail, num_stores, self.configs.numStqEntries)
        ...
        ldq_tail_oh = LogicVec(ctx, 'ldq_tail_oh', 'w', self.configs.numLdqEntries)
        arch += BitsToOH(ctx, ldq_tail_oh, ldq_tail)
        ldq_head_next_oh = LogicVec(
            ctx, 'ldq_head_next_oh', 'w', self.configs.numLdqEntries)
        ldq_head_next = LogicVec(ctx, 'ldq_head_next', 'w', self.configs.ldqAddrW)
        ...
        stq_tail_oh = LogicVec(ctx, 'stq_tail_oh', 'w', self.configs.numStqEntries)
        arch += BitsToOH(ctx, stq_tail_oh, stq_tail)
        stq_head_next_oh = LogicVec(
            ctx, 'stq_head_next_oh', 'w', self.configs.numStqEntries)
        stq_head_next = LogicVec(ctx, 'stq_head_next', 'w', self.configs.stqAddrW)
        ...
        if self.configs.pipe0:
            ...
            if self.configs.pipeComp:
                ...
                can_load = LogicArray(ctx, 'can_load', 'w', self.configs.numLdqEntries)
        ...
        if self.configs.pipe1:
            ...
            load_en_p1 = LogicArray(ctx, 'load_en_p1', 'r', self.configs.numLdMem)

            load_hs = LogicArray(ctx, 'load_hs', 'w', self.configs.numLdMem)
            load_p1_ready = LogicArray(ctx, 'load_p1_ready', 'w', self.configs.numLdMem)

            store_idx_p1 = LogicVec(ctx, 'store_idx_p1', 'r', self.configs.stqAddrW)
            ...
            for i in range(0, self.configs.numLdqEntries):
                ...
                read_data = LogicVec(ctx, f'read_data_{i}', 'w', self.configs.dataW)
                for w in range(0, self.configs.numLdMem):
                ...
                bypass_data = LogicVec(ctx, f'bypass_data_{i}', 'w', self.configs.dataW)
