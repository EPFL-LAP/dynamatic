...
class GroupAllocator(BaseVHDLGenerator):
    ...
    def generate(self) -> None:
        ...
        ldq_wen_o = LogicArray(self.ctx, 'ldq_wen', 'o', self.configs.numLdqEntries)
        num_loads_o = LogicVec(self.ctx, 'num_loads', 'o', self.configs.ldqAddrW)
        ...
        stq_wen_o = LogicArray(self.ctx, 'stq_wen', 'o', self.configs.numStqEntries)
        num_stores_o = LogicVec(self.ctx, 'num_stores', 'o', self.configs.stqAddrW)
        ...
        stores_sub = LogicVec(self.ctx, 'stores_sub', 'w', self.configs.stqAddrW)
        empty_loads = LogicVec(self.ctx, 'empty_loads', 'w', self.configs.emptyLdAddrW)
        empty_stores = LogicVec(self.ctx, 'empty_stores', 'w', self.configs.emptyStAddrW)
        ...
        group_init_hs = LogicArray(self.ctx, 'group_init_hs', 'w', self.configs.numGroups)
        ...
        if (self.configs.gaMulti):
            ...
            ga_rr_mask = LogicVec(self.ctx, 'ga_rr_mask', 'r', self.configs.numGroups)
            ga_rr_mask.regInit()
            for i in range(0, self.configs.numGroups):
            ...
            for i in range(0, self.configs.numGroups):
                arch += Op(self.ctx, group_init_ready_o[i], group_init_ready[i])
        ...
        arch += Mux1HROM(self.ctx, num_loads, self.configs.gaNumLoads, group_init_hs)
        arch += Mux1HROM(self.ctx, num_stores, self.configs.gaNumStores, group_init_hs)
        ...
        arch += CyclicLeftShift(self.ctx, ldq_wen_o, ldq_wen_unshifted, ldq_tail_i)
        arch += CyclicLeftShift(self.ctx, stq_wen_o, stq_wen_unshifted, stq_tail_i)
        for i in range(0, self.configs.numLdqEntries):
            arch += CyclicLeftShift(self.ctx,
                                    ga_ls_order_temp[i], ga_ls_order_rom[i], stq_tail_i)
        arch += CyclicLeftShift(self.ctx, ga_ls_order_o, ga_ls_order_temp, ldq_tail_i)
    ...
    def instantiate(
        ...
        arch = self.ctx.get_current_indent() + f'{self.module_name} : entity work.{self.module_name}\n'
        self.ctx.tabLevel += 1
