...
private:
  LogicalResult parseOptions(Preprocessor &PP, const Token &FirstToken,
    ...
    // NOTE: Copied more or less from the PredicPragmaHandler, common
    // abstraction pattern for parsing in values maybe?
    ...
    // NOTE: Right now all are enforced, not needed in the future. e.g. factor
    // is obsolote in the full case or when we have an automatism to determine
    // ideal factor
