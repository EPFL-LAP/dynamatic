...
from generators.support.signal_manager.utils.concat import (
    get_concat_extra_signals_bitwidth,
)
...
def generate_init(name, params):
    ...
    if extra_signals:
        return _generate_init_signal_manager(
            name, bitwidth, extra_signals, initial_value
        )
    if bitwidth == 0:
...
def _generate_init(name, bitwidth, initial_value):
    ...
    dependencies = _generate_init_dataless(init_dataless_name)
    data_reg_init = f"'{initial_value}'"
...
def _generate_init_signal_manager(name, bitwidth, extra_signals, initial_value):
    ...
    return generate_concat_signal_manager(
        ...
        extra_signals,
        lambda name: _generate_init(
            name, bitwidth + extra_signals_bitwidth, initial_value
        ),
    )
