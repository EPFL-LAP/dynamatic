; ModuleID = '/home/shundroid/dynamatic/integration-test/golden_ratio/out_1_exit/comp/clang_optnone_removed.ll'
source_filename = "/home/shundroid/dynamatic/integration-test/golden_ratio/golden_ratio.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

; Function Attrs: noinline nounwind uwtable
define dso_local float @golden_ratio(float noundef %x0, float noundef %x1) #0 {
entry:
  br label %for.body

for.body:                                         ; preds = %entry, %for.inc
  %i.03 = phi i32 [ 0, %entry ], [ %inc, %for.inc ]
  %original_x.02 = phi float [ %x1, %entry ], [ %div, %for.inc ]
  %x.01 = phi float [ %x0, %entry ], [ %add4, %for.inc ]
  br label %while.body

while.body:                                       ; preds = %while.body, %for.body
  %x.1 = phi float [ %x.01, %for.body ], [ %mul1, %while.body ]
  %mul = fmul float %x.1, %original_x.02
...
entry:
  %call = call float @golden_ratio(float noundef 1.000000e+02, float noundef 0x3F847AE140000000)
  ret i32 0
}
