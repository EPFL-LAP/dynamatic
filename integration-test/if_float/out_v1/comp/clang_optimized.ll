; ModuleID = '/home/shundroid/dynamatic/integration-test/if_float/out_v1/comp/clang_optnone_removed.ll'
source_filename = "/home/shundroid/dynamatic/integration-test/if_float/if_float.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

; Function Attrs: noinline nounwind uwtable
define dso_local float @if_float(float noundef %x0, ptr noundef %a, ptr noundef %minus_trace) #0 {
entry:
  br label %for.body

for.body:                                         ; preds = %entry, %for.inc
  %i.02 = phi i32 [ 0, %entry ], [ %inc, %for.inc ]
  %x.01 = phi float [ %x0, %entry ], [ %add6, %for.inc ]
...
if.then:                                          ; preds = %for.body
  %mul3 = fmul float %x.01, 0x3FF19999A0000000
  br label %for.inc
...
if.else:                                          ; preds = %for.body
  ...
  %div = fdiv float %x.01, 0x3FF19999A0000000
  br label %for.inc

for.inc:                                          ; preds = %if.then, %if.else
  %x.1 = phi float [ %mul3, %if.then ], [ %div, %if.else ]
  %add6 = fadd float %x.1, %x.1
  %inc = add nuw nsw i32 %i.02, 1
  %cmp = icmp ult i32 %inc, 100
  br i1 %cmp, label %for.body, label %for.end, !llvm.loop !6

for.end:                                          ; preds = %for.inc
  %x.0.lcssa = phi float [ %add6, %for.inc ]
  ret float %x.0.lcssa
}
...
for.body:                                         ; preds = %entry, %for.body
  ...
  %idxprom1 = zext i32 %j.01 to i64
  %arrayidx2 = getelementptr inbounds [100 x float], ptr %minus_trace, i64 0, i64 %idxprom1
...
for.end:                                          ; preds = %for.body
  %call4 = call float @if_float(float noundef 1.000000e+02, ptr noundef nonnull %a, ptr noundef nonnull %minus_trace)
  ret i32 0
}
