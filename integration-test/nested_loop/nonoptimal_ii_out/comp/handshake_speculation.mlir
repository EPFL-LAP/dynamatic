module {
  handshake.func @nested_loop(%arg0: memref<1000xi32>, %arg1: memref<1000xi32>, %arg2: memref<1000xi32>, %arg3: !handshake.control<>, %arg4: !handshake.control<>, %arg5: !handshake.control<>, %arg6: !handshake.control<>, ...) -> (!handshake.control<>, !handshake.control<>, !handshake.control<>, !handshake.control<>) attributes {argNames = ["a", "b", "c", "a_start", "b_start", "c_start", "start"], resNames = ["a_end", "b_end", "c_end", "end"]} {
    ...
    %memEnd = mem_controller[%arg2 : memref<1000xi32>] %arg5 (%39, %addressResult_5, %dataResult_6) %73#2 {connectedBlocks = [2 : i32], handshake.name = "mem_controller3"} :    (!handshake.channel<i32>, !handshake.channel<i10>, !handshake.channel<i32>) -> ()
    ...
    %13 = mux %41#3 [%12, %43] {handshake.bb = 2 : ui32, handshake.name = "mux1"} : <i1>, [<i32>, <i32>] to <i32>
    ...
    %18 = mux %41#2 [%4#1, %44] {handshake.bb = 2 : ui32, handshake.name = "mux2"} : <i1>, [<i2>, <i2>] to <i2>
    %19 = mux %41#1 [%11, %42] {handshake.bb = 2 : ui32, handshake.name = "mux3"} : <i1>, [<i12>, <i12>] to <i12>
    ...
    %22 = mux %41#0 [%6#1, %45] {handshake.bb = 2 : ui32, handshake.name = "mux4"} : <i1>, [<>, <>] to <>
    ...
    %addressResult_5, %dataResult_6 = store[%38] %37 {handshake.bb = 2 : ui32, handshake.deps = #handshake<deps[<"store1" (0)>]>, handshake.mem_interface = #handshake.mem_interface<MC>, handshake.name = "store1"} : <i10>, <i32>, <i10>, <i32>
    %35 = cmpi slt, %33#1, %31 {handshake.bb = 2 : ui32, handshake.name = "cmpi0"} : <i32>
    %36 = not %57#0 {handshake.bb = 2 : ui32, handshake.name = "not0"} : <i1>
    %37 = passer %33#0[%56#3] {handshake.bb = 2 : ui32, handshake.name = "passer0"} : <i32>, <i1>
    %38 = passer %34[%56#2] {handshake.bb = 2 : ui32, handshake.name = "passer1"} : <i10>, <i1>
    %39 = passer %25[%56#1] {handshake.bb = 2 : ui32, handshake.name = "passer2"} : <i32>, <i1>
    %40 = init %47#5 {handshake.bb = 2 : ui32, handshake.name = "init0"} : <i1>
    %41:4 = fork [4] %40 {handshake.bb = 2 : ui32, handshake.name = "fork18"} : <i1>
    %42 = passer %20#1[%47#4] {handshake.bb = 2 : ui32, handshake.name = "passer3"} : <i12>, <i1>
    %43 = passer %58[%47#3] {handshake.bb = 2 : ui32, handshake.name = "passer4"} : <i32>, <i1>
    %44 = passer %59#1[%47#2] {handshake.bb = 2 : ui32, handshake.name = "passer5"} : <i2>, <i1>
    %45 = passer %23#2[%47#1] {handshake.bb = 2 : ui32, handshake.name = "passer6"} : <>, <i1>
