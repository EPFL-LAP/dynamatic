...
def resize_fifo(test_dir, base_name, fifo_size, seed):
    ...
    if simulate_test(test_dir, base_name, log_file) != 0:
        log("[FATAL] Initial simulation failed.", log_file)
        return False
    baseline_cycles = parse_cycles(test_dir / SIM_DIR / "report.txt")
    log(f"Initial BASELINE Cycles: {baseline_cycles}", log_file)
    if baseline_cycles is None:
        log("[FATAL] Could not get initial baseline cycle count.", log_file)
        return False
    ...
    if not buffer_cp(MAX_CP, test_dir, log_file):
        return False
    if not canonicalize_handshake(test_dir, log_file):
        return False
    if not lower_handshake_to_hw(test_dir, log_file):
        return False
    ...
    try:
        if simulate_test(test_dir, base_name, log_file) != 0:
            log("[FATAL] Baseline simulation failed.", log_file)
            return False
    ...
    try:
        if simulate_test(test_dir, base_name, log_file) != 0:
            log("[FATAL] Final baseline simulation failed.", log_file)
            return False
        baseline_cycles = parse_cycles(sim_log)
    except Exception:
        log("[FATAL] Baseline simulation failed. Cannot proceed with optimization.", log_file)
...
def check_performance(test_dir: Path, basename: str, baseline_cycles: int) -> bool:
    ...
    try:
        if simulate_test(test_dir, basename, log_file) != 0:
            log("[ERROR] Simulation failed.", log_file)
            return False
...
def optimize_buffer_resize(test_dir: Path, basename: str, buffer_list: list, baseline_cycles: int, target_size, fifo_size, results, processed_num, skip) -> list:
    global total_num
    ...
    if not skip and check_performance(test_dir, basename, baseline_cycles):
        ...
        rem_buffs_1, p_num_1, is_good_1 = optimize_buffer_resize(test_dir, basename, first_half, baseline_cycles, target_size, fifo_size, results, processed_num, False)
        resized_buffers.extend(rem_buffs_1)
        rem_buffs_2, p_num_2, is_good_2 = optimize_buffer_resize(test_dir, basename, second_half, baseline_cycles, target_size, fifo_size, results, p_num_1, is_good_1)
