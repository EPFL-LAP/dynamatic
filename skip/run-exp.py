...
def find_best_target_cp(test_dir: Path, basename: str, log_file: Path):
    ...
    if simulate_test(test_dir, basename, log_file) != 0:
        log("Initial target-CP simulation failed.", log_file)
        return None
    sim_out = test_dir / SIM_DIR / "report.txt"
    baseline_cycles = parse_cycles(sim_out)
    if baseline_cycles is None:
        log("Could not parse initial target-CP latency.", log_file)
        return None

    with open(log_file, "a") as log:
        log.write(f"Baseline latency = {baseline_cycles} cycles at {MAX_CP} ns\n")
    ...
    while cp >= 3.0:
        ...
        if not buffer_cp(cp, test_dir, log_file):
            return None
        ...
        # Simulate
        if simulate_test(test_dir, basename, log_file) != 0:
            log(f"Simulation failed for CP={cp}.", log_file)
            return None
...
def find_best_timing(test_dir: Path, basename: str, start_cp:float, n: int, log_file: Path):
    ...
    while target_cp >= 3.0:
        ...
        if not buffer_cp(target_cp, test_dir, log_file):
            return False
        if not canonicalize_handshake(test_dir, log_file):
            return False
        if not lower_handshake_to_hw(test_dir, log_file):
        ...
        if simulate_test(test_dir, basename, log_file) != 0:
            log(f"Simulation failed for CP={target_cp}.", log_file)
            return False
        sim_out = test_dir / SIM_DIR / "report.txt"
        cycle = parse_cycles(sim_out)
        ...
        if real_cp is None:
            log(f"Could not determine real CP for target {target_cp}.", log_file)
            return False
        cps.append(real_cp)
...
def run_pipeline(test_name: str, n: int, seed, run_root: Path):
    ...
    if compile(test_out_dir, basename, MAX_CP, n, MAX_FIFO_SIZE, log_file) != 0:
        log("[FATAL] Initial compilation failed.", log_file)
        return False

    if not resize_fifo(test_out_dir, basename, MAX_FIFO_SIZE, seed):
        log("[FATAL] FIFO resizing failed.", log_file)
        return False


    best_target_cp = find_best_target_cp(test_out_dir, basename, log_file)

    if best_target_cp is None:
        log("[FATAL] Target CP tuning failed.", log_file)
        return False
    start_cp = best_target_cp
    log(f"Starting from {start_cp} ns", log_file)

    if not find_best_timing(test_out_dir, basename, start_cp, n, log_file):
        log("[FATAL] Timing exploration failed.", log_file)
        return False
...
def main():
    ...
    for test, n, seed in inputs:
        succeeded = run_pipeline(test, n, seed, run_root) and succeeded
    if not succeeded:
        sys.exit(1)
