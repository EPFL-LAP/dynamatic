...
def buffer_cp(cp, test_dir: Path, log_file: Path):
    ...
    cmd = (
        ...
        f'--handshake-set-unit-impl-attr="target-period={cp} '
        f'timing-models={timing_models} impl=flopoco" '
        ...
        f'timing-models={timing_models} '
        f'spec-timing-models={Path(DYNAMATIC_PATH).parent.parent / "data/spec-timing.json"} '
        f'target-period={cp} timeout=300 dump-milp-models" '
        f'> "{handshake_buffered}"'
    )

    rc = run_cmd(cmd, cwd=None, timeout=TIMEOUT * 60, log_file=log_file)
    if rc != 0:
        log(f"Buffering failed for CP={cp}", log_file)
        return False
    if not handshake_buffered.exists() or handshake_buffered.stat().st_size == 0:
        log(f"Buffering produced no MLIR for CP={cp}", log_file)
        return False
    if handshake_buffered.read_text(errors="ignore").startswith("OVERVIEW:"):
        log(f"Buffering emitted optimizer help instead of MLIR for CP={cp}", log_file)
        return False
    return True
...
def canonicalize_handshake(test_dir: Path, log_file: Path):
    ...
    if rc != 0:
        ...
        return False
    if not f_export.exists() or f_export.stat().st_size == 0:
        log("Canonicalization produced no MLIR.", log_file)
        return False
...
def lower_handshake_to_hw(test_dir: Path, log_file: Path):
    ...
    if rc != 0:
        ...
        return False
    if not f_hw.exists() or f_hw.stat().st_size == 0:
        log("Handshake-to-HW lowering produced no MLIR.", log_file)
        return False
