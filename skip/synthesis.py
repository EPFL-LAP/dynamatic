...
def place_and_route(test_dir: Path, iter_dir: Path, basename: str, constraints: Path, iter_log_file: Path):
    ...
    rc = run_cmd(cmd, cwd=None, timeout=TIMEOUT * 60, log_file=iter_log_file)
    expected_reports = (
        iter_dir / "timing_post_pr.rpt",
        iter_dir / "utilization_post_pr.rpt",
    )
    if rc == 0 and not all(report.exists() for report in expected_reports):
        log("Vivado exited without producing post-route reports.", iter_log_file)
        return 1
    return rc
