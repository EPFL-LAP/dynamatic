module {
  handshake.func @gaussian(%arg0: memref<20xi32>, %arg1: memref<400xi32>, %arg2: !handshake.control<>, %arg3: !handshake.control<>, %arg4: !handshake.control<>, ...) -> (!handshake.channel<i32>, !handshake.control<>, !handshake.control<>, !handshake.control<>) attributes {argNames = ["c", "a", "c_start", "a_start", "start"], cfg.edges = "[0,1][2,3,5,cmpi2][4,2][1,2][3,3,4,cmpi0][5,1,6,cmpi1]", resNames = ["out0", "c_end", "a_end", "end"]} {
    ...
    %addressResult, %dataResult = load[%42] %0#0 {handshake.bb = 3 : ui32, handshake.deps = #handshake<deps[["store0", 0, true]]>, handshake.name = "load0"} : <i32>, <i32>, <i32>, <i32>
    ...
    %addressResult_16, %dataResult_17 = load[%46] %0#1 {handshake.bb = 3 : ui32, handshake.deps = #handshake<deps[["store0", 0, true]]>, handshake.name = "load2"} : <i32>, <i32>, <i32>, <i32>
    ...
    %addressResult_18, %dataResult_19, %doneResult = store[%52] %48 %0#2 {handshake.bb = 3 : ui32, handshake.deps = #handshake<deps[["load0", 0, true], ["load2", 0, true], ["store0", 0, true]]>, handshake.name = "store0"} : <i32>, <i32>, <>, <i32>, <i32>, <>
    ...
    end {handshake.bb = 6 : ui32, handshake.name = "end0"} %75, %memEnd, %0#3, %arg4 : <i32>, <>, <>, <>
  }
}
