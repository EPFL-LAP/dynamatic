module {
  ...
  ^bb2:  // pred: ^bb1
    ...
    %3 = memref.load %arg0[%1] {handshake.name = "load3"} : memref<1000xi32>
    %4 = memref.load %arg1[%1] {handshake.name = "load4"} : memref<1000xf32>
    %5 = arith.index_cast %3 {handshake.name = "index_cast1"} : i32 to index
    %6 = memref.load %arg2[%5] {handshake.deps = #handshake<deps[["store0", 0, true]]>, handshake.name = "load2"} : memref<1000xf32>
    %7 = arith.addf %6, %4 {handshake.name = "addf0"} : f32
    memref.store %7, %arg2[%5] {handshake.deps = #handshake<deps[["load2", 0, true], ["store0", 0, true]]>, handshake.name = "store0"} : memref<1000xf32>
