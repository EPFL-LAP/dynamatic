module {
  ...
  ^bb1(%0: index):  // 2 preds: ^bb0, ^bb5
    %1 = memref.load %arg3[%0] {handshake.deps = #handshake<deps[["store3", 2, true]]>, handshake.name = "load5"} : memref<20xf32>
  ...
  ^bb2(%2: index, %3: f32):  // 2 preds: ^bb1, ^bb2
    ...
    %6 = memref.load %arg0[%5] {handshake.name = "load10"} : memref<400xf32>
    %7 = memref.load %arg1[%2] {handshake.name = "load7"} : memref<20xf32>
  ...
  ^bb3:  // pred: ^bb2
    cf.br ^bb4(%c0 : index) {handshake.name = "br2"}
  ^bb4(%12: index):  // 2 preds: ^bb3, ^bb4
    %13 = memref.load %arg2[%12] {handshake.deps = #handshake<deps[["store2", 1, true], ["store2", 3, true]]>, handshake.name = "load8"} : memref<20xf32>
    %14 = arith.muli %0, %c20 {handshake.name = "muli1"} : index
    %15 = arith.addi %12, %14 {handshake.name = "addi1"} : index
    %16 = memref.load %arg0[%15] {handshake.name = "load11"} : memref<400xf32>
    %17 = arith.mulf %16, %9 {handshake.name = "mulf1"} : f32
    %18 = arith.addf %13, %17 {handshake.name = "addf1"} : f32
    memref.store %18, %arg2[%12] {handshake.deps = #handshake<deps[["load8", 1, true], ["store2", 1, true]]>, handshake.name = "store2"} : memref<20xf32>
    %19 = arith.addi %12, %c1 {handshake.name = "addi3"} : index
    %20 = arith.cmpi ult, %19, %c20 {handshake.name = "cmpi1"} : index
    cf.cond_br %20, ^bb4(%19 : index), ^bb5 {handshake.name = "cond_br1"}
  ^bb5:  // pred: ^bb4
    memref.store %9, %arg3[%0] {handshake.name = "store3"} : memref<20xf32>
    %21 = arith.addi %0, %c1 {handshake.name = "addi4"} : index
    %22 = arith.cmpi ult, %21, %c20 {handshake.name = "cmpi2"} : index
    cf.cond_br %22, ^bb1(%21 : index), ^bb6 {handshake.name = "cond_br2"}
  ^bb6:  // pred: ^bb5
