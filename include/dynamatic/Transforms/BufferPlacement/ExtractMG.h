...
/// to the map.
int extractCFDFCircuit(std::map<ArchBB *, unsigned> &archs, std::map<int, bool> &bbs);
...
bool isSelect(std::map<int, bool> &bbs, Value *val);

/// Identify whether the channel is in selected archs between the basic block.
bool isSelect(std::map<ArchBB *, unsigned> &archs, Value *val);
...
LogicalResult readSimulateFile(const std::string &fileName,
                      std::map<ArchBB *, unsigned> &archs, std::map<int, bool> &bbs);
} // namespace buffer
} // namespace dynamatic
