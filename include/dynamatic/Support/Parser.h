...
// methods for parsing sum-of-products (SOP) expressions using a lexical analyzer 
// to generate tokens and a stack-based approach to construct the parse tree.
class Parser {
public:
	LexicalAnalyzer lexer;

    // Function that implements the main parsing logic, using the precedence table to guide token processing and reduction.
...
private:
    // This function ensures the correct terminal node is used for precedence comparison.
...
StackNode* constructOperatorStackNode(StackNode* operate,  StackNode* s1,  StackNode* s2);

// handle NOT operations in the parsing stack.
StackNode* constructNegatorStackNode(StackNode* s1);

// reduce a sequence of tokens based on operator precedence and returns the resulting stack node.
