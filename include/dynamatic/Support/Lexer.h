...
//===----------------------------------------------------------------------===//
//
// This file defines the data structure for handling lexical analysis, specifically 
// for a set of tokens used in boolean logic expressions.
// The LexicalAnalyzer class provides methods for initializing with an input string, 
...
enum class TokenType : int {
    NotToken, AndToken, OrToken, LparenToken, RparenToken, VariableToken, EndToken
}; 

// Struct representing an individual token, consisting of a lexeme and a token type.
struct Token {
    std::string lexeme; // The string representation of the token.
    TokenType tokenType; // The type of the token.

    // Default constructor initializing lexeme to empty string and token type to EndToken.
...
private:
    std::vector<Token> tokenList; // List of tokens extracted from the input expression.
