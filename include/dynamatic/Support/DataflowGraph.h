...
public:
  ...
  /// Self-loops (srcBB == dstBB) are valid transitions.
  static std::vector<std::vector<dynamatic::experimental::ArchBB>>
  enumerateSequences(
      const std::vector<dynamatic::experimental::ArchBB> &transitions,
      unsigned sequenceLength);

  /// Build the dataflow graph from a single sequence of consecutive transitions.
  /// The sequence should have been generated by enumerateSequences.
  /// For a sequence of N transitions, the graph will have N+1 steps.
  void buildGraphFromSequence(
  ...
  /// Dump multiple dataflow graphs to a single GraphViz file.
  /// Each graph is placed in its own cluster subgraph.
  static void dumpAllGraphsToFile(const std::vector<DataflowGraph> &graphs,
  ...
  static void dumpAllReconvergentPathsToFile(
