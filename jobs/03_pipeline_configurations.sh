...
# headlag only with at least two pipeline stages, otherwise it doesn't make sense
# TODO: check this is valid, critical path is not the head pointer update loop

export SYNTHESIS_CLOCK_PERIOD_NS="2.5"

OUTPUT_DIR="${SCRIPT_DIR}/../eval_results/03_pipeline_configurations_${GIT_REVISION}"
mkdir -p "${OUTPUT_DIR}"
cat <<EOF >"${OUTPUT_DIR}/README.txt"
Experiment 3: Pipeline Configurations
