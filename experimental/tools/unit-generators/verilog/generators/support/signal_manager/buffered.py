...
def _generate_transfer_logic(in_channels: list[Channel], out_channels: list[Channel]) -> tuple[str, str]:
    ...
    Assignments example:
        assign transfer_in = lhs_valid & lhs_ready;
        assign transfer_out = result_valid & result_ready;
    """

    first_in_channel_name = in_channels[0]["name"]
    first_out_channel_name = out_channels[0]["name"]

    return f"""assign transfer_in = {first_in_channel_name}_valid & {first_in_channel_name}_ready;
  assign transfer_out = {first_out_channel_name}_valid & {first_out_channel_name}_ready;""", \
        "wire transfer_in, transfer_out;"
...
def _generate_concat(concat_layout: ConcatLayout) -> tuple[str, str]:
    ...
    # Example: wire [0:0] signals_pre_buffer;
    ...
    # Example:
    # assign signals_pre_buffer[0:0] = forwarded_spec;
    ...
    for out_channel_name in out_channel_names:
        for signal_name, _ in extra_signals.items():
            output_assignments.append(
                f"assign {out_channel_name}_{signal_name} = sliced_{signal_name};")
