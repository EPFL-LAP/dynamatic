...
def get_default_extra_signal_value(extra_signal_name: str):
    """
    Return the default Verilog value for an extra signal
...
def generate_forwarding_expression_for_signal(signal_name: str, in_extra_signal_names: list[str]) -> str:
    """
    Generate a Verilog expression to forward an extra signal
    ...
    if signal_name == "spec":
        return " | ".join(in_extra_signal_names)
