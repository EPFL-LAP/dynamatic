from generators.support.signal_manager import generate_buffered_arith_signal_manager
from generators.handshake.join import generate_join
from generators.support.delay_buffer import generate_delay_buffer
from generators.handshake.oehb import generate_oehb


def generate_fpu_wrapper(name, params):
    is_double = params["is_double"]
    internal_delay = params["internal_delay"]
    latency = params["latency"]
    extra_signals = params["extra_signals"]

    core_unit = params["core_unit"]


    bitwidth = 64 if is_double else 32
    if extra_signals:
        generate_inner = lambda name: _generate_wrapper(name, bitwidth, core_unit, internal_delay, latency)
        return generate_buffered_arith_signal_manager(
            name,
            bitwidth,
            extra_signals,
            generate_inner,
        ...
        return _generate_wrapper(name, bitwidth, core_unit, internal_delay, latency)



def _generate_wrapper(name, bitwidth, core_unit, internal_delay, latency):
...
architecture arch of {name} is
  ...
  begin
    ...
    operator : entity work.FloatingPoint{core_unit}_{bitwidth}_{internal_delay}(arch)
    port map (
        clk   => clk,
        {clock_enables}
        X     => ip_lhs,
