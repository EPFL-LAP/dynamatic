...
def generate_mulf(name, params):
    ...
    if extra_signals:
        return _generate_mulf_signal_manager(name, bitwidth, internal_delay, latency, extra_signals)
    else:
        return _generate_mulf(name, bitwidth, internal_delay, latency)



def _generate_mulf(name, bitwidth, internal_delay, latency):
...
architecture arch of {name} is
  ...
  begin
    ...
    operator : entity work.FloatingPointMultiplier_{bitwidth}_{internal_delay}(arch)
    port map (
        clk   => clk,
        {clock_enables}
        X     => ip_lhs,
...
def _generate_mulf_signal_manager(name, bitwidth, internal_delay, latency, extra_signals):
    return generate_buffered_signal_manager(
        ...
        extra_signals,
        lambda name: _generate_mulf(name, bitwidth, internal_delay, latency),
        latency)
