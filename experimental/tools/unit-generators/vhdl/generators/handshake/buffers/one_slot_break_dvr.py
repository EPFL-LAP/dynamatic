...
def generate_one_slot_break_dvr(name, params):
    ...
    if extra_signals:
        return _generate_one_slot_break_dvr_signal_manager(name, bitwidth, extra_signals, debug_counter)
    if bitwidth == 0:
        return _generate_one_slot_break_dvr_dataless(name, debug_counter)
    else:
        return _generate_one_slot_break_dvr(name, bitwidth, debug_counter)


def _generate_one_slot_break_dvr_dataless(name, debug_counter):
...
def _generate_one_slot_break_dvr(name, bitwidth, debug_counter):
...
def _generate_one_slot_break_dvr_signal_manager(name, bitwidth, extra_signals, debug_counter):
    ...
    return generate_concat_signal_manager(
        ...
        extra_signals,
        lambda name: _generate_one_slot_break_dvr(name, bitwidth + extra_signals_bitwidth, debug_counter))
