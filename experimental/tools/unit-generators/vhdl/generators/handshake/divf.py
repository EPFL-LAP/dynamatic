...
def _generate_divf(name, is_double):
    if is_double:
        return _generate_divf_double_precision(name)
    else:
        return _generate_divf_single_precision(name)


def _get_latency(is_double):
    return 36 if is_double else 29  # todo


def _generate_divf_single_precision(name):
...
def _generate_divf_double_precision(name):
    join_name = f"{name}_join"
    oehb_name = f"{name}_oehb"
    buff_name = f"{name}_buff"
...
-- Entity of divf_double_precision
entity {name} is
  port (
    -- inputs
    clk          : in std_logic;
...
"""
    ...
    architecture = f"""
-- Architecture of divf_double_precision
architecture arch of {name} is
  signal join_valid                         : std_logic;
  signal buff_valid, oehb_valid, oehb_ready : std_logic;
