...
def generate_divf(name, params):
    ...
    if extra_signals:
        return _generate_divf_signal_manager(name, bitwidth, internal_delay, latency, extra_signals)
    else:
        return _generate_divf(name, bitwidth, internal_delay, latency)



def _generate_divf(name, bitwidth, internal_delay, latency):
...
"""
    ...
    clock_enables = "\n".join(
        [f"        ce_{i} => oehb_ready," for i in range(1, n)]
        )
...
architecture arch of {name} is
  ...
  begin
    ...
    operator : entity work.FloatingPointDivider_{bitwidth}_{internal_delay}(arch)
    port map (
        clk   => clk,
        {clock_enables}
        X     => ip_lhs,
