module {
  handshake.func @b_lhs(%x: !handshake.channel<i32>, %y: !handshake.channel<i32>, %d: !handshake.control<>, %c: !handshake.channel<i1>, ...) -> (!handshake.channel<i32>, !handshake.control<>) attributes {argNames = ["Xd", "Yd","Dd", "Cd"], resNames = ["A", "B"]} {
    ...
    %result, %index = control_merge %d, %f_0_buf {handshake.bb = 1 : ui32, handshake.name = "cmerge"} : <>, <i1>
    ...
    %f_0_buf = buffer %f_0 {handshake.bb = 1 : ui32, handshake.name = "buf", hw.parameters = {NUM_SLOTS = 1 : ui32, TIMING = #handshake<timing {D: 1, V: 1, R: 0}>}} : <>
