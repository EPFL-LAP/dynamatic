module {
  handshake.func @f_rhs(%d: !handshake.channel<i32>, %m: !handshake.channel<i1>, %n: !handshake.channel<i1>, ...) -> (!handshake.channel<i32>) attributes {argNames = ["D", "M", "N"], resNames = ["A"]} {
    ...
    %ctrl = mux %n [%m, %const_1] {handshake.bb = 1 : ui32, handshake.name = "ctrl_mux"}  : <i1>, <i1>
    ...
    %loop_out = buffer %f {handshake.bb = 1 : ui32, handshake.name = "comb_buf", hw.parameters = {NUM_SLOTS = 1 : ui32, TIMING = #handshake<timing {D: 1, V: 1, R: 0}>}} : <i32>
