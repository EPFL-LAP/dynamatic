module {
  handshake.func @sup_mux_rhs(%loopLiveIn: !handshake.channel<i1>, %iterLiveOut: !handshake.channel<i1>, %oldContinue: !handshake.channel<i1>, ...) -> (!handshake.channel<i1>) attributes {argNames = ["loopLiveIn", "iterLiveOut", "oldContinue"], resNames = ["iterLiveIn"]} {
    %newContinue = spec_v2_repeating_init %oldContinue {handshake.bb = 1 : ui32, handshake.name = "ri1", initToken = 1 : ui1} : <i1>
    ...
    %newContinue2 = spec_v2_repeating_init %oldContinue {handshake.bb = 1 : ui32, handshake.name = "ri2", initToken = 1 : ui1} : <i1>
    %passerOut = passer %data [%newContinue2] {handshake.bb = 1 : ui32, handshake.name = "passer"} : <i1>, <i1>
