module {
  handshake.func @sup_gamma_mux2_rhs(%cond: !handshake.channel<i1>, %dataT: !handshake.channel<i1>, %dataF: !handshake.channel<i1>, ...) -> (!handshake.channel<i1>) attributes {argNames = ["cond", "dataT", "dataF"], resNames = ["res"]} {
    ...
    %cond_not = not %cond_forked#2 {handshake.name = "not"} : <i1>
    %dataF_passed = passer %dataF [%cond_not] {handshake.name = "dataF_passer"} : <i1>, <i1>
    %data_muxed = mux %ri_forked#2 [%dataF_passed, %dataT] {handshake.name = "data_mux"} : <i1>, [<i1>, <i1>] to <i1>
