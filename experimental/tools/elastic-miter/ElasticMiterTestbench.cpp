...
static std::string createMiterProperties(const std::string &moduleName,
  ...
  for (const auto &[resultName, resultType] : config.results) {
    if (auto resultChannelType = dyn_cast<handshake::ChannelType>(resultType)) {
      assert(resultChannelType.getDataBitWidth() == 1 &&
             "The output channel must be generated by the comparison op with "
             "the bitwidth of 1.");
  ...
  for (const auto &[i, buffers] : llvm::enumerate(config.outputBuffers)) {
    ...
    //       llvm::formatv("(!{0}.{1}.outs_valid)", moduleName,
    //       lhsBlocker).str());
    //   bufferProperties.push_back(
    //       llvm::formatv("(!{0}.{1}.outs_valid)", moduleName,
    //       rhsBlocker).str());
