module {
  hw.module @switchSuppressCtrl_rhs(in %val : !handshake.channel<i1>, in %A_in : !handshake.channel<i1>, in %clk : i1, in %rst : i1, out B_out : !handshake.channel<i1>) {
    ...
    hw.instance "vm_sink_0" @handshake_sink_0(ins: %fork.outs_2: !handshake.channel<i1>, clk: %clk: i1, rst: %rst: i1) -> ()
    %passer.result = hw.instance "passer" @handshake_passer_0(data: %ndw_in_A_in.outs: !handshake.channel<i1>, ctrl: %interpolator.result: !handshake.channel<i1>, clk: %clk: i1, rst: %rst: i1) -> (result: !handshake.channel<i1>)
  ...
  hw.module @switchSuppressCtrl_rhs_wrapper(in %val : !handshake.channel<i1>, in %A_in : !handshake.channel<i1>, in %clk : i1, in %rst : i1, out B_out : !handshake.channel<i1>) {
    %switchSuppressCtrl_rhs_wrapped.B_out = hw.instance "switchSuppressCtrl_rhs_wrapped" @switchSuppressCtrl_rhs(val: %val: !handshake.channel<i1>, A_in: %A_in: !handshake.channel<i1>, clk: %clk: i1, rst: %rst: i1) -> (B_out: !handshake.channel<i1>)
    hw.output %switchSuppressCtrl_rhs_wrapped.B_out : !handshake.channel<i1>
  }
}
