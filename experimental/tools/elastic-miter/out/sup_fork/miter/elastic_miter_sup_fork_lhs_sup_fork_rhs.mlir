module {
  handshake.func @elastic_miter_sup_fork_lhs_sup_fork_rhs(%arg0: !handshake.channel<i1>, %arg1: !handshake.channel<i1>, ...) -> (!handshake.channel<i1>, !handshake.channel<i1>) attributes {argNames = ["A_in", "Cond"], resNames = ["EQ_B_out", "EQ_C_out"]} {
    ...
    %6 = buffer %5#0, bufferType = FIFO_BREAK_DV, numSlots = 1 {debugCounter = true, handshake.bb = 0 : ui32, handshake.name = "lhs_in_buf_Cond"} : <i1>
    %7 = buffer %5#1, bufferType = FIFO_BREAK_DV, numSlots = 1 {debugCounter = true, handshake.bb = 0 : ui32, handshake.name = "rhs_in_buf_Cond"} : <i1>
    ...
    %12 = buffer %11#0, bufferType = FIFO_BREAK_DV, numSlots = 1 {debugCounter = false, handshake.bb = 5 : ui32, handshake.name = "out_buf_lhs_nds_B_out"} : <>
    %13 = buffer %11#1, bufferType = FIFO_BREAK_DV, numSlots = 1 {debugCounter = false, handshake.bb = 5 : ui32, handshake.name = "out_buf_rhs_nds_B_out"} : <>
    ...
    %23 = buffer %22#0, bufferType = FIFO_BREAK_DV, numSlots = 1 {debugCounter = false, handshake.bb = 5 : ui32, handshake.name = "out_buf_lhs_nds_C_out"} : <>
    %24 = buffer %22#1, bufferType = FIFO_BREAK_DV, numSlots = 1 {debugCounter = false, handshake.bb = 5 : ui32, handshake.name = "out_buf_rhs_nds_C_out"} : <>
