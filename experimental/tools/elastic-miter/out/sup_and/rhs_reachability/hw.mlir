module {
  hw.module @sup_and_rhs(in %A_in : !handshake.channel<i1>, in %B_in : !handshake.channel<i1>, in %Cond : !handshake.channel<i1>, in %clk : i1, in %rst : i1, out C_out : !handshake.channel<i1>) {
    ...
    %ndw_out_C_out.outs = hw.instance "ndw_out_C_out" @handshake_ndwire_0(ins: %passer1.result: !handshake.channel<i1>, clk: %clk: i1, rst: %rst: i1) -> (outs: !handshake.channel<i1>)
    %andi.result = hw.instance "andi" @handshake_andi_0(lhs: %ndw_in_A_in.outs: !handshake.channel<i1>, rhs: %ndw_in_B_in.outs: !handshake.channel<i1>, clk: %clk: i1, rst: %rst: i1) -> (result: !handshake.channel<i1>)
    %passer1.result = hw.instance "passer1" @handshake_passer_0(data: %andi.result: !handshake.channel<i1>, ctrl: %ndw_in_Cond.outs: !handshake.channel<i1>, clk: %clk: i1, rst: %rst: i1) -> (result: !handshake.channel<i1>)
