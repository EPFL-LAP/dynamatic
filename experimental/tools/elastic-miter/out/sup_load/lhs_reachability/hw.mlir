module {
  hw.module @sup_load_lhs(in %Addr_in : !handshake.channel<i1>, in %Cond : !handshake.channel<i1>, in %clk : i1, in %rst : i1, out Data_out : !handshake.channel<i1>) {
    %passer.result = hw.instance "passer" @handshake_passer_0(data: %Addr_in: !handshake.channel<i1>, ctrl: %Cond: !handshake.channel<i1>, clk: %clk: i1, rst: %rst: i1) -> (result: !handshake.channel<i1>)
    ...
    hw.output %load0.dataOut : !handshake.channel<i1>
  }
