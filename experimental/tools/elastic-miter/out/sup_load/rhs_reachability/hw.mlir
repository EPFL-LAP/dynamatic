module {
  hw.module @sup_load_rhs(in %Addr_in : !handshake.channel<i1>, in %Cond : !handshake.channel<i1>, in %clk : i1, in %rst : i1, out Data_out : !handshake.channel<i1>) {
    ...
    %ndw_out_Data_out.outs = hw.instance "ndw_out_Data_out" @handshake_ndwire_0(ins: %passer.result: !handshake.channel<i1>, clk: %clk: i1, rst: %rst: i1) -> (outs: !handshake.channel<i1>)
    %load0.addrOut, %load0.dataOut = hw.instance "load0" @handshake_load_0(addrIn: %ndw_in_Addr_in.outs: !handshake.channel<i1>, dataFromMem: %mc_buffer.outs: !handshake.channel<i1>, clk: %clk: i1, rst: %rst: i1) -> (addrOut: !handshake.channel<i1>, dataOut: !handshake.channel<i1>)
    ...
    %passer.result = hw.instance "passer" @handshake_passer_0(data: %load0.dataOut: !handshake.channel<i1>, ctrl: %ndw_in_Cond.outs: !handshake.channel<i1>, clk: %clk: i1, rst: %rst: i1) -> (result: !handshake.channel<i1>)
  ...
  hw.module @sup_load_rhs_wrapper(in %Addr_in : !handshake.channel<i1>, in %Cond : !handshake.channel<i1>, in %clk : i1, in %rst : i1, out Data_out : !handshake.channel<i1>) {
    %sup_load_rhs_wrapped.Data_out = hw.instance "sup_load_rhs_wrapped" @sup_load_rhs(Addr_in: %Addr_in: !handshake.channel<i1>, Cond: %Cond: !handshake.channel<i1>, clk: %clk: i1, rst: %rst: i1) -> (Data_out: !handshake.channel<i1>)
    hw.output %sup_load_rhs_wrapped.Data_out : !handshake.channel<i1>
  }
}
