...
MOD = "z"
F_HANDSHAKE_MITER = os.path.join(COMP_DIR, f"ndwirererd_{MOD}_lhs.mlir")
...
def main():
  # TODO put this stuff into functions
  # Convert to DOT
  cmd = ["bin/export-dot", f"{REWRITES}/{MOD}_lhs.mlir", "--edge-style=spline"]
  ...
  ret = run_nuXmv(os.path.join(COMP_DIR, "prove_inf.cmd"), os.path.join(OUT_DIR, "inf_states.txt"))
  ...
  while True:
      ...
      smv_output = os.path.join(OUT_DIR, f"comp/main_{N}.smv")
      ...
      nr_of_differences = get_states(os.path.join(OUT_DIR, "inf_states.txt"), os.path.join(OUT_DIR, f"{N}_states.txt"))
