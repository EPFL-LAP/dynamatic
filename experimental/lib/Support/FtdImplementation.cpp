...
void ftd::addRegenOperandConsumer(PatternRewriter& rewriter,
  ...
  // skip
  if (consumerOp->hasAttr(FTD_REGEN) || consumerOp->hasAttr(FTD_EXPLICIT_MU_PHI) ||
    consumerOp->hasAttr(FTD_EXPLICIT_GAMMA_PHI) ||
...
static Value addSuppressionInLoop(PatternRewriter& rewriter, CFGLoop* loop,
  ...
  if (Block* loopExit = loop->getExitingBlock(); loopExit) {
    ...
    // Detected by the producer being in the same block or a later block after the loop exit
    if (btlt == BackwardRelationship &&
      (bi.isGreater(connection.getParentBlock(), loopExit) ||
        (connection.getParentBlock() == loopExit && connection.getDefiningOp()->hasAttr(FTD_EXPLICIT_GAMMA_PHI))
        ))
      return connection;
...
/// Identify the block that the muxCondition serves as the terminator condition for
...
static Block* returnMuxConditionBlock(Value muxCondition) {
  ...
  muxCondition.getDefiningOp()->print(llvm::errs());
  ...
  assert(muxConditionBlock && "Mux condition must be feeding any block terminator.");
...
static void insertDirectSuppression(
  ...
  if (producerBlock == consumerBlock &&
    (consumer->getAttr(FTD_EXPLICIT_GAMMA_PHI) && connection.getDefiningOp()->getAttr(FTD_EXPLICIT_MU_PHI) ||
      consumer->getAttr(FTD_EXPLICIT_MU_PHI) && connection.getDefiningOp()->getAttr(FTD_EXPLICIT_GAMMA_PHI)
      )
    )
    fCons =
    enumeratePaths(consumerBlock, consumerBlock, bi, consControlDeps);
  else
    fCons =
  ...
  if (accountMuxCondition) {
    ...
    BoolExpression* selectOperandCondition = BoolExpression::parseSop(
      bi.getBlockCondition(muxConditionBlock));
...
void ftd::addSuppOperandConsumer(PatternRewriter& rewriter,
  ...
  if (Operation* producerOp = operand.getDefiningOp(); producerOp) {
    // skip delivery if both the producer and consumer are Muxes implementing GAMMAs
    if (consumerBlock == producerBlock && consumerOp->getAttr(FTD_EXPLICIT_GAMMA_PHI)
...
LogicalResult experimental::ftd::addGsaGates(Region& region,
  ...
  for (Block& block : llvm::drop_begin(region)) {
    ...
    for (Gate* phi : phis) {
      ...
      if (phi->gsaGateFunction == MuGate)
        mux->setAttr(FTD_EXPLICIT_MU_PHI, rewriter.getUnitAttr());
      else
        mux->setAttr(FTD_EXPLICIT_GAMMA_PHI, rewriter.getUnitAttr());
    }
  }
