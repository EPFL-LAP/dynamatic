...
void ResourceSharingInfo::computeOccupancySum() { return; }

void Group::addOperation(mlir::Operation *op) { items.push_back(op); }

bool Group::recursivelyDetermineIfCyclic(
    mlir::Operation *current_op, std::set<mlir::Operation *> &node_visited,
    mlir::Operation *op) {
...
void Set::addGroup(Group group) { groups.push_back(group); }

void Set::joinGroups(GroupIt group1, GroupIt group2,
                     std::vector<mlir::Operation *> &finalOrd) {
  Group newly_created =
      Group(finalOrd, group1->shared_occupancy + group1->shared_occupancy,
...
void ResourceSharing::retrieveDataFromPerformanceAnalysis(
    ResourceSharingInfo sharing_feedback, std::vector<int> &SCC,
    int number_of_SCC, TimingDatabase timingDB) {
...
void ResourceSharing::print() {
  llvm::errs() << "\n***** Basic Blocks *****\n";
  for (auto arch_item : archs) {
    llvm::errs() << "Source: " << arch_item.srcBB
                 << ", Destination: " << arch_item.dstBB << "\n";
  }
...
void ResourceSharing::placeAndComputeNecessaryDataFromPerformanceAnalysis(
    ResourceSharingInfo data, TimingDatabase timingDB) {
...
LogicalResult
MyFPGA20Buffers::addSyncConstraints(std::vector<Value> opaqueChannel) {
...
void findBBEdges(std::deque<std::pair<int, int>> &BBops,
  ...
  while (end != size) {
    ...
    while (end != size &&
           getLogicBB(permutation_vector[end]).value() == BasicBlockId) {
      ++end;
    }
...
bool get_next_permutation(PermutationEdge begin_of_permutation_vector,
  ...
  for (auto [start, end] : separation_of_BBs) {
    if (next_permutation(begin_of_permutation_vector + start,
                         begin_of_permutation_vector + end)) {
      return true;
    }
  }
  return false;
...
Value generate_performance_step(OpBuilder *builder, mlir::Operation *op,
  ...
  mlir::Value control_merge =
      control_map[getLogicBB(op).value()].current_position;
  ...
  mlir::Operation *syncOp =
      builder->create<SyncOp>(control_merge.getLoc(), controlOperands);
...
generate_performance_model(OpBuilder *builder,
  ...
  for (auto op : items) {
    return_values.push_back(
        generate_performance_step(builder, op, control_map));
  }
