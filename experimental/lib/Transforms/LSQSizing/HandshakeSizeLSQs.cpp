...
void HandshakeSizeLSQsPass::runDynamaticPass() {
  ...
  for (handshake::FuncOp funcOp : mod.getOps<handshake::FuncOp>()) {
    ...
    for (auto &maxLoadStoreSize : maxLoadStoreSizes) {
      ...
      if (maxLoadSize < 2) {
        llvm::dbgs() << " [DBG] LSQ " << getUniqueName(lsqOp).str()
                     << " Load Size: " << maxLoadSize
                     << " is too small, setting to 2\n";
      }
      if (maxStoreSize < 2) {
        llvm::dbgs() << " [DBG] LSQ " << getUniqueName(lsqOp).str()
                     << " Store Size: " << maxStoreSize
                     << " is too small, setting to 2\n";
      }
...
HandshakeSizeLSQsPass::findStartTimes(AdjListGraph graph) {
  ...
  for (auto &entry : maxLatencies) {
    ...
    llvm::dbgs()
        << "\t [DBG] Start Node Candidate: "
        << entry.first->getAttrOfType<StringAttr>("handshake.name").str()
        << " Shifting: " << maxLatency - entry.second << "\n";
  }
...
void HandshakeSizeLSQsPass::insertStartnodeShiftingEdges(
  ...
  for (auto &entry : startTimes) {
    ...
    if (get<0>(entry) != startNode) {
      ...
      if (paths.size() == 0) {
        llvm::dbgs()
            << "\t [DBG] Inserting Shifting Edge from "
            << startNode->getAttrOfType<StringAttr>("handshake.name").str()
            << " to "
            << get<0>(entry)->getAttrOfType<StringAttr>("handshake.name").str()
            << " with latency " << get<1>(entry) << "\n";
        graph.addShiftingEdge(startNode, get<0>(entry), get<1>(entry));
      } else {
        llvm::dbgs()
            << "\t [DBG] Shifting not necessary: " << paths.size()
            << " paths found between "
            << startNode->getAttrOfType<StringAttr>("handshake.name").str()
            << " and "
            << get<0>(entry)->getAttrOfType<StringAttr>("handshake.name").str()
