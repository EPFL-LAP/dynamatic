...
namespace {
  ...
  struct CombineShannonMuxes : public OpRewritePattern<handshake::MuxOp> {
  ...
  // having a bwd edge
  struct CombineRegenerationMuxes : public OpRewritePattern<handshake::MuxOp> {
    using OpRewritePattern<handshake::MuxOp>::OpRewritePattern;
    LogicalResult matchAndRewrite(handshake::MuxOp muxOp,
      PatternRewriter& rewriter) const override {
      ...
      // traversal and returning its produced value to deal with a tree of regeneration Muxes
  ...
  struct HandshakeCombineSteeringLogicPass
    ...
    void runDynamaticPass() override {
      ...
      patterns.add<RemoveSinkMuxes, RemoveDoubleSinkBranches,
        ...
        CombineInits, CombineRegenerationMuxes, CombineShannonMuxes, RemoveNotCondition>(ctx);
