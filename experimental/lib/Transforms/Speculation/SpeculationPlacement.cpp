...
SpeculationPlacements::readFromAttribute(mlir::ModuleOp modOp,
  ...
  mlir::Value res;
  if (auto loadOp = mlir::dyn_cast<handshake::LoadOp>(producer)) {
    res = loadOp.getDataResult();
  } else if (producer->getNumResults() == 1) {
    res = producer->getResult(0);
  } else {
    producer->emitError() << "`dynamatic.speculate` producer must have "
                             "exactly one result (or be a handshake.load)";
    return failure();
  }
  if (!res.hasOneUse()) {
