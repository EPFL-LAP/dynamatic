...
/// Returns whether the operand has a cyclic path within its BB by performing
/// DFS.
static bool hasCyclicPathInBBRecursive(OpOperand &operand, OpOperand &current,
                                       llvm::DenseSet<OpOperand *> &visited,
                                       unsigned bb) {
  if (visited.count(&current)) {
    // Traversal detected another cycle that does not include the operand.
    return false;
  }
  ...
  // Perform DFS traversal to detect a cyclic path.
...
/// Determines whether the operand has a cyclic path within its BB.
static bool hasCyclicPathInBB(OpOperand &operand) {
  ...
  if (!bb) {
    ...
    llvm_unreachable("hasCyclicPathInBB Failed");
  }
    ...
    if (hasSpecInput && hasNonSpecInput) {
      for (OpOperand &operand : blockOp->getOpOperands()) {
        ...
        if (!specValues.contains(operand.get())) {
          ...
          // Mitigation: all save units are later converted to save-commit
          // units. Save-commit units are only placed on cyclic paths.
