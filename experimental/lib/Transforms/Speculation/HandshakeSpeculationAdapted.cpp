...
#define GEN_PASS_DEF_HANDSHAKESPECULATIONADAPTED
...
struct HandshakeSpeculationAdaptedPass
    : public dynamatic::experimental::speculation::impl::
          HandshakeSpeculationAdaptedBase<HandshakeSpeculationAdaptedPass> {

  using HandshakeSpeculationAdaptedBase<
      HandshakeSpeculationAdaptedPass>::HandshakeSpeculationAdaptedBase;
...
static void routeCommitControlRecursive(
    MLIRContext *ctx, SpecPreBufferOp2 &specOp,
    llvm::DenseSet<Operation *> &arrived, OpOperand &currOpOperand,
    std::vector<BranchTracingItem> &branchTrace, bool passedInnerMux) {
  ...
  if (auto commitOp = dyn_cast<handshake::SpecCommitOp>(currOp)) {
    ...
    for (auto [valueForSpecTag, branchOp, branchDirection] : branchTrace) {
      if (passedInnerMux && branchOp->hasAttr("specv1_adaptor_inner_loop")) {
        llvm::errs() << "skipping branch inside inner loop\n";
        continue;
      }
    ...
    auto branchOp = cast<handshake::ConditionalBranchOp>(currOp);
...
LogicalResult HandshakeSpeculationAdaptedPass::routeCommitControl() {
  ...
  for (auto saveCommitOp :
    ...
    for (OpOperand &succOpOperand : saveCommitOp.getDataOut().getUses()) {
      ...
      routeCommitControlRecursive(&getContext(), specOp2, arrived,
                                  succOpOperand, branchTrace, false);
    }
  }
...
LogicalResult HandshakeSpeculationAdaptedPass::placeCommits() {
...
LogicalResult
HandshakeSpeculationAdaptedPass::placeSaveCommits(Value ctrlSignal) {
...
static handshake::ConditionalBranchOp findControlBranch(FuncOp funcOp,
  ...
  for (auto condBrOp : funcOp.getOps<handshake::ConditionalBranchOp>()) {
    if (auto brBB = getLogicBB(condBrOp); !brBB || brBB != bb)
      continue;
    if (condBrOp->hasAttr("specv1_adaptor_inner_loop"))
      continue;
...
FailureOr<Value> HandshakeSpeculationAdaptedPass::generateSaveCommitCtrl() {
  MLIRContext *ctx = &getContext();
  OpBuilder builder(ctx);
...
static std::optional<Value> findControlInputToBB(handshake::FuncOp &funcOp,
                                                 unsigned targetBB) {
  ...
  for (auto controlMergeOp : funcOp.getOps<handshake::ControlMergeOp>()) {
    ...
    if (auto brBB = getLogicBB(controlMergeOp); !brBB || brBB != targetBB)
      continue;

    // BB should have only one control branch at most
    if (isControlBranchFound) {
      controlMergeOp->emitError("Multiple control branches found in the BB #" +
                                std::to_string(targetBB));
      return {};
    }
    triggerChannelOrigin = controlMergeOp.getResult();
...
LogicalResult HandshakeSpeculationAdaptedPass::placeSpeculator() {
  MLIRContext *ctx = &getContext();
...
addSpecTagToSpecRegionRecursive(MLIRContext &ctx, OpOperand &opOperand,
  ...
  if ((isa<handshake::ControlMergeOp>(op) || isa<handshake::MuxOp>(op)) &&
      !op->hasAttr("specv1_adaptor_inner_loop")) {
    if (isDownstream) {
...
LogicalResult HandshakeSpeculationAdaptedPass::addSpecTagToSpecRegion() {
  llvm::DenseSet<Operation *> visited;
...
LogicalResult HandshakeSpeculationAdaptedPass::addNonSpecOp() {
...
void HandshakeSpeculationAdaptedPass::runDynamaticPass() {
