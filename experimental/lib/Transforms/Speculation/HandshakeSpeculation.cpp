...
LogicalResult HandshakeSpeculationPass::placeCommits() {
  ...
  fakeControlForCommits =
      mlir::UnrealizedConversionCastOp::create(
          builder, specOp->getLoc(), commitCtrl.getType(), ValueRange{})
          .getResult(0);
...
FailureOr<Value> HandshakeSpeculationPass::generateSaveCommitCtrl() {
  ...
  auto branchDiscardCondNonSpec = handshake::SpeculatingBranchOp::create(
      builder, controlBranch.getLoc(),
      /*trueResultType=*/conditionOperand.getType(),
      /*falseResultType=*/conditionOperand.getType(),
  ...
  auto branchDiscardCondNonMisspec = handshake::ConditionalBranchOp::create(
      builder, branchDiscardCondNonSpec.getLoc(), specOp.getSCIsMisspec(),
  ...
  auto branchReplicated = handshake::ConditionalBranchOp::create(
      builder, branchDiscardCondNonMisspec.getLoc(),
  ...
  auto mergeOp = handshake::MergeOp::create(builder, branchReplicated.getLoc(),
                                            mergeOperands);
...
LogicalResult HandshakeSpeculationPass::placeSpeculator() {
  ...
  // resultType is tentative and will be updated in the addSpecTag algorithm
  // later.
  specOp = handshake::SpeculatorOp::create(
      builder, dstOp->getLoc(), /*resultType=*/srcOpResult.getType(),
