...
void insertNewlineBeforeStyle(const string &inputFileName,
...
void removeBackslashWithSpaceFromPos(const string &inputFileName,
                                     const string &outputFileName) {
  ifstream inputFile(inputFileName);
  ofstream outputFile(outputFileName);
  ...
  while (getline(inputFile, line)) {
    ...
    if (posPos != string::npos && backslashPos != string::npos &&
        posPos < backslashPos) {
      string modifiedLine =
          line.substr(0, backslashPos) + line.substr(backslashPos + 2);
...
void reformatDot(const std::string &inputFileName,
                 const std::string &outputFileName) {
  // std::cout << "enter reformat"<< std::endl;
