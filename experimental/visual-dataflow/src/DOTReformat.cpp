...
LogicalResult reformatDot (const std::string& inputFile, const std::string& outputFile) {
  ...
  while (getline(inFile, line)) {
      ...
      if (isEdge) {
          ...
          if (line.find("pos=\"") != std::string::npos) {
              ...
              // If the line does not end with a backslash, it's a single-line pos attribute
              if (line.back() != '\\') {
                  isPosLine = false;
