...
- Generation Pattern: A signal that is conceptually an array in the source code (e.g., `port_bits_o`) is "unrolled" into multiple, distinct signals in the VHDL entity. The generated VHDL signals are indexed with a suffix, such as `port_bits_{p}_o`, where `{p}` is the port index.

- Interpreting Diagrams: If a diagram or conceptual description uses a base name without an index (e.g., `port_bits_o`), it represents a collection of signals. The actual dimension is expanded based on the context:

    - Port-related signals (like `port_bits_o`) are expanded by the number of ports (`N_PORTS`).
...
| **Outputs** | | | | |
| `port_bits_o` | `port_bits_{p}_o` | Output | `std_logic_vector(PAYLOAD_WIDTH-1:0)` | Data payload sent to port `p`. |
| `port_valid_o` | `port_valid_{p}_o` | Output | `std_logic` | Valid flag for port `p`. Asserted to indicate that `port_bits_{p}_o` contains valid data. |
...
| `entry_bits_i` | `entry_bits_{e}_i` | Input | `std_logic_vector(PAYLOAD_WIDTH-1:0)` | The data stored in queue entry `e`. |
...
| **Outputs** | | | | |
| `entry_reset_o` | `entry_reset_{e}_o` | Output | `std_logic` | Reset signal for an entry. `entry_reset_{e}_o` is asserte
.d to deallocate entry `e` after its data has been successfully sent. |
...
![Find Valid Entry](./figs/qtp/QTP_Find_Valid_Entry.png)   
    This block identifies which entries in the queue are currently valid (`entry_alloc_{e}_i` = `1`), meaning whether each entry is allocated by the group allocator or not.
    - **Input**:  
    ...
    - **Processing**:  
        ...
        - If an entry is not valid (i.e., not allocated, `entry_alloc_{e}_i = 0`), its port assignment is masked, resulting in a zero vector.
        - If the entry is valid (i.e., allocated, `entry_alloc_{e}_i = 1`), its one-hot port assignment is passed through unchanged.
    - **Output**:  
        - `entry_waiting_for_port`: The resulting matrix where a `1` at position `(e,p)` indicates that entry `e` is valid and assigned to port `p`. This matrix represents all potential candidates for sending data and is fed into the arbitration logic `CyclicPriorityMasking` to determine which entry gets to send its data first for each port.
...
![Find Oldest Valid Entry](./figs/qtp/QTP_Find_Oldest_Valid_Entry.png)   
    ...
    - **Input**:  
        - `entry_waiting_for_port`: A matrix where a `1` at position (e, p) indicates that queue entry `e` is valid and assigned to port `p`. This represents the entire pool of candidates competing for access to the output ports.
        - `queue_head_oh_i`: The queue's one-hot head vector, which represents the priority (i.e., the oldest entry) for the current cycle.
    - **Processing**:
        - It uses a **CyclicPriorityMasking** algorithm, which operates on each port (column of `entry_waiting_for_port`). 
    ...
    - **Output**:  
        - `entry_waiting_for_port_prio`: The resulting matrix after arbitration. For each port (column), this matrix now contains at most `1` (it's a one-hot vector or all zeros). This `1` indicates the single, highest-priority entry that has won the arbitration for that port.
...
![Payload Mux](./figs/qtp/QTP_Payload_Mux.png)  
    ...
    - **Input**:  
        - `entry_bits_i`: `N_ENTRIES` of the data payload from all queue entries.
        - `entry_waiting_for_port_prio`: The arbitrated selection matrix from the **Find Oldest Valid Entry** block. For each port (column), this matrix contains at most a single `1`, which identifies the oldest winning entry for that port.
    - **Processing**:  
        - For each output port `p`, a one-hot multiplexer (`Mux1H`) uses the `p`-th column of the `entry_waiting_for_port_prio` matrix as its select signal.
        - This operation selects the data payload from the single oldest entry out of the entire `entry_bits_i` and routes it to the corresponding output port.
    - **Output**:  
        - `port_bits_o`: `N_PORTS` of the data payloads. `port_bits_{p}_o` holds the data from the oldest winning queue entry for that port, ready to be sent to the external access port.
...
![Handshake Logic](./figs/qtp/QTP_Handshake_Logic.png)  
    ...
    - **Input**:  
        ...
        - `entry_waiting_for_port_prio`: The arbitrated selection matrix from the **Find Oldest Valid Entry** block, indicating at most the single winning entry for each port.
    - **Processing**:  
        - **Check Winner's Data Validity**: First, the block verifies if the data in the winning entry is actually ready. It masks the `entry_waiting_for_port_prio` matrix with the `entry_payload_valid_i`. If the winning entry for a port doesn't have valid data, it is nullified for this cycle. The result is `entry_waiting_for_port_valid`.
...
Based on the example diagram:  
    ...
    - The logic then combines this validity information with the decoded port index for each entry (`entry_port_idx_oh` from the `Port Index Decoder`). An entry's one-hot port information is passed through only if its corresponding `entry_alloc_i` bit is `1`.
    - If an entry is not valid (like `Entry 0`), its output for this stage is zeroed out (`000`).
    - The result is the `entry_waiting_for_port` matrix, which represents the initial list of all allocated queue entries and their target ports. This matrix is then sent to the `Find Oldest Valid Entry` block for arbitration.
...
Based on the example diagram:  
    ...
    - The result is `entry_waiting_for_port_prio`
...
Based on the example diagram:  
    - First, the logic checks the `entry_payload_valid_i` vector, which is `[0, 1, 1, 0]`. This indicates that among the winning queue entries, data is valid and ready to be sent from `Entry 1` and `Entry 2`.
    - For the `Port 0` winner (`Entry 2`), its `entry_payload_valid_i` is `1`. The logic asserts `port_valid_o[0]` to `1`.
    - For the `Port 2` winner (`Entry 1`), its `entry_payload_valid_i` is `1`. The logic asserts `port_valid_o[2]` to `1`.
