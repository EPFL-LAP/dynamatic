...
Both the operator specific wrappers and the shared flopoco reference file are generated by the [seperate unit module generator]([waiting on seeing final location to add links](https://github.com/ETHZ-DYNAMO/flopoco-to-dataflow-converter). See its own documentation for further details.
