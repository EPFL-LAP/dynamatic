...
The legacy Dynamatic backend supports the existence of multiple implementations of a given operator, by allowing an "arch-name" to be specified. This system is leveraged by having each floating point wrapper file (addf.vhd, mulf.vhd, etc.) contain a seperate arch for each FloPoCo implementation, identified by a bitwidth-delay pair, added as a suffix to "arch" to form a unique name. The implementations are contained in a [seperate file](https://github.com/EPFL-LAP/dynamatic/blob/main/data/vhdl/support/flopoco_ip_cores.vhd).

Both of these files are generated by the seperate unit module generator (waiting on seeing final location to add links). See its own documentation for further details.
