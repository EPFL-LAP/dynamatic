...
The producer and consumer can sit in several positions relative to the loops of the CFG, and the construction has to handle all of them with one set of rules:

<img src="./Figures/ch5_loop_classification.png" width="820"/>

*Figure 2: The delivery shapes, classified by how the producer block `x = ...` and the consumer block `... = x` sit relative to a loop (back edges in blue). (a) Same loop, producer before consumer: the token is delivered along a forward path within one iteration, and taking the back edge instead would make the producer fire again, so the back edge means suppression. (b) Same loop, but the consumer is the loop header, reachable only by taking the back edge: this is a loop-carried delivery to the internal input of a μ-gate or a regeneration mux. (c) Producer inside a loop, consumer outside it: the token is delivered only on the iteration that exits the loop, and the producer's earlier iterations are suppressed. (d, e) Producer and consumer are both outside a loop that lies between them. The two cases differ by whether the consumer post-dominates the loop header. In (d) it does, so the loop cannot change whether the consumer is reached and its internal branches drop out of the suppression condition. In (e) it does not, so a branch inside the loop decides whether the consumer is reached; the loop may run many iterations, and only the last condition token matters. Both (d) and (e) put the loop on a producer-to-consumer path, so the raw local CFG keeps it as a real cycle, but only (e) actually needs the cyclic machinery: in (d) the loop drops out once the decision graph keeps only the deciding branches.*
